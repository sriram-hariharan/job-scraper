...
Phase 47B adds a controlled exact resume change-set manual review packet builder dry-run command.

The command reads supplied normalized change proposals file, reads supplied normalization result file, and reads supplied review context file. It calls the Phase 47A controlled exact resume change-set manual review packet builder and prints manual review packet JSON to stdout.

It builds manual review packets after Phase 46 normalization. It is not a provider call phase. It is not a validation phase. It is not a normalization phase. It is not a UI write phase. It prepares packets for later UI/manual review readback.

The command preserves manual review and user acceptance requirements, and produces manual-review proposal-only output. It does not create new proposal text.
...
```bash
PYTHONPATH="$PWD" python run_controlled_exact_resume_change_set_manual_review_packet_builder_dry_run.py --input path/to/normalized_change_proposals.json --review-context path/to/review_context.json
```
