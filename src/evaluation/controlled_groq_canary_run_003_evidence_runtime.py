"""Dedicated one-row durable evidence runtime for Groq canary run 003."""
...
from src.evaluation.controlled_groq_canary_run_003_identity import (
    ...
    run_003_authorization_template_sha256,
    run_003_identity_sha256,
    validate_run_003_authorization_template,
...
from src.evaluation.controlled_groq_canary_run_003_transport import (
...
RUN_003_EVIDENCE_RUNTIME_VERSION = (
    "controlled-groq-canary-run-003-evidence-runtime-v1"
)
RUN_003_CHECKPOINT_SCHEMA_VERSION = (
    "controlled-groq-canary-run-003-checkpoint-v1"
)
RUN_003_RESULT_SCHEMA_VERSION = "controlled-groq-canary-run-003-result-v1"
...
def _validated_owners() -> tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    plan = build_run_003_plan_contract()
    identity = build_run_003_identity_contract()
    transport = build_run_003_transport_contract()
    validate_run_003_plan_contract(plan)
    validate_run_003_identity_contract(identity)
    validate_run_003_transport_contract(transport)
    _require(
    ...
    _require(
        run_003_authorization_template_sha256()
        == _PINNED_AUTHORIZATION_TEMPLATE_SHA256,
        "run-003 authorization-template digest changed",
    )
    _require(
        len(plan["schedule"]) == 1
        and identity["schedule"] == plan["schedule"]
        and transport["schedule"] == plan["schedule"]
        and plan["schedule"][0]["schedule_key"] == _PINNED_SCHEDULE_KEY,
        "run-003 schedule ownership changed",
    )
    return deepcopy(plan), deepcopy(identity), deepcopy(transport)


def validate_run_003_active_authorization(
    ...
    *,
    ...
    _require(
        ...
        "active run-003 authorization is required",
    )
    inactive = build_run_003_authorization_template()
    validate_run_003_authorization_template(inactive)
    _require(
        set(authorization) == set(inactive),
        "run-003 authorization fields must match the exact template",
    )
    for key in set(inactive) - _ACTIVE_AUTHORIZATION_FIELDS:
        _require(
            authorization[key] == inactive[key],
            f"run-003 authorization changed fixed field {key}",
        )
...
def _bindings(
    *,
    ...
    plan, identity, _transport = _validated_owners()
    return {
        "run_003_evidence_runtime_version": (
            RUN_003_EVIDENCE_RUNTIME_VERSION
        ),
        "checkpoint_schema_version": RUN_003_CHECKPOINT_SCHEMA_VERSION,
        ...
        "run_003_transport_sha256": run_003_transport_sha256(),
        ...
        "schedule_keys": [_PINNED_SCHEDULE_KEY],
...
def _empty_aggregate() -> Dict[str, Any]:
    return {
        ...
        "by_schedule_key": {_PINNED_SCHEDULE_KEY: 0},
    }
...
def build_empty_run_003_checkpoint(
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
    execution_at_utc: str,
) -> Dict[str, Any]:
    validate_run_003_active_authorization(
        authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
    )
...
def _state_sets(checkpoint: Mapping[str, Any]) -> tuple[set[str], ...]:
    ...
    for field in _STATE_FIELDS:
        ...
        _require(
            ...
            and set(values) <= {_PINNED_SCHEDULE_KEY},
            f"{field} contains invalid run-003 keys",
        )
        observed.append(set(values))
    for index, left in enumerate(observed):
        for right in observed[index + 1 :]:
            _require(left.isdisjoint(right), "run-003 state lists overlap")
...
def _validate_summary(summary: Dict[str, Any]) -> bool:
    _require(
        ...
        "run-003 grading summary fields are invalid",
    )
    _require(
        summary["schedule_key"] == _PINNED_SCHEDULE_KEY
        and summary["workload_id"] == "skill_extraction"
        and summary["provider"] == "groq"
        and summary["model"] == "openai/gpt-oss-120b",
        "run-003 grading summary ownership changed",
    )
    _require(
        ...
        "run-003 grading booleans are invalid",
    )
    ...
    _require(
        ...
        "run-003 hard failures are not bounded positive counters",
    )
    _require(
        summary["provider_outcome_category"] == "success",
        "run-003 persisted outcome category is invalid",
    )
...
def _recomputed_aggregate(
    *,
    ...
    aggregate["by_workload"]["skill_extraction"] = count
    aggregate["by_schedule_key"][_PINNED_SCHEDULE_KEY] = count
    if summaries:
        summary = summaries[0]
        ...
        aggregate["observed_cost"] = summary["observed_cost"]
    return aggregate


def validate_run_003_checkpoint(
    ...
    *,
    ...
    if not invoked:
        _require(
            ...
            "empty run-003 checkpoint state is inconsistent",
        )
    elif completed:
        _require(
            len(summaries) == 1
            ...
            "completed run-003 checkpoint is inconsistent",
        )
    elif hard:
        _require(
            ...
            and (
                summaries == []
                or summaries[0]["quality_gate_passed"] is False
            ),
            "hard-failure run-003 checkpoint is inconsistent",
        )
    elif blocked:
        _require(
            summaries == []
            ...
            and checkpoint["cost_comparison_eligibility"] is False,
            "blocked run-003 checkpoint is inconsistent",
        )
    elif ambiguous:
        _require(
            summaries == []
            ...
            and checkpoint["quality_gate_status"] == "stopped"
            and checkpoint["cost_comparison_eligibility"] is False,
            "ambiguous run-003 checkpoint is inconsistent",
        )
    ...
    _require(
        ...
        "run-003 aggregate usage is inconsistent",
    )
    _require(
        checkpoint["authority_invariants"] == _expected_authority(len(invoked)),
        "run-003 checkpoint authority changed",
    )
    return True


def get_next_run_003_row(
    checkpoint: Dict[str, Any],
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
    ...
    if invoked:
        return None
    return deepcopy(checkpoint["schedule"][0])
...
def _validate_transition_start(
    ...
    *,
    ...
    next_row = get_next_run_003_row(
        checkpoint,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
    )
    _require(next_row is not None, "run-003 checkpoint is terminal")
    _require(
        isinstance(scheduled, Mapping) and dict(scheduled) == next_row,
        "run-003 transition row is not the exact next row",
    )
...
def _terminal_transition(
    ...
    *,
    ...
    updated[state_field] = [_PINNED_SCHEDULE_KEY]
    updated["grading_summaries"] = [] if summary is None else [deepcopy(summary)]
    invoked = {_PINNED_SCHEDULE_KEY}
    updated["aggregate_usage"] = _recomputed_aggregate(
        invoked=invoked,
        summaries=updated["grading_summaries"],
    )
    updated["stop_reason"] = stop_reason
    updated["quality_gate_status"] = quality_status
    updated["cost_comparison_eligibility"] = (
        state_field == "completed_schedule_keys"
    )
    updated["authority_invariants"] = _expected_authority(1)
    validate_run_003_checkpoint(
        updated,
        authorization=authorization,
        pricing=pricing,
...
def record_run_003_blocked_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    authorization: Dict[str, Any],
...
def record_run_003_ambiguous_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    authorization: Dict[str, Any],
    ...
    _require(
        ...
        "run-003 ambiguous reason is not approved",
    )
    return _terminal_transition(
        checkpoint,
...
def record_run_003_hard_failure_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    authorization: Dict[str, Any],
...
def record_run_003_completed_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    ...
    _require(
        ...
        "run-003 transport result fields are invalid",
    )
    _require(
        transport_result["provider_outcome_category"] == "success",
        "run-003 completed transition requires a success outcome",
    )
    ...
    _require(
        ...
        "run-003 transport provider/model mismatch",
    )
    ...
    _require(
        ...
        and output_tokens > 0,
        "run-003 positive token usage is required",
    )
    ...
    _require(len(matches) == 1, "run-003 fixture ownership changed")
    case = matches[0]
    ...
    token_exceeded = input_tokens > 4096 or output_tokens > 1024
    if token_exceeded:
    ...
    cost_exceeded = (
        observed_cost > model_ceiling or observed_cost > total_ceiling
    )
    if cost_exceeded:
    ...
    if not passed:
        ...
        return record_run_003_hard_failure_call(
            checkpoint,
            scheduled=row,
            authorization=authorization,
            pricing=pricing,
...
def serialize_run_003_checkpoint(
    checkpoint: Dict[str, Any],
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
...
def build_run_003_result_artifact(
    *,
    checkpoint: Dict[str, Any],
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
    ...
    _require(sum(counts.values()) == 1, "result requires terminal run-003 state")
    if counts["completed"]:
    ...
    artifact = {
        "result_schema_version": RUN_003_RESULT_SCHEMA_VERSION,
        "run_003_evidence_runtime_version": (
            RUN_003_EVIDENCE_RUNTIME_VERSION
        ),
        "run_identifier": RUN_003_IDENTIFIER,
        ...
        "authorization_sha256": _canonical_sha256(authorization),
        "pricing_sha256": pricing_table_sha256(pricing),
        "artifact_paths": deepcopy(RUN_003_ARTIFACT_PATHS),
    ...
    validate_run_003_result_artifact(
        artifact,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
...
def validate_run_003_result_artifact(
    ...
    *,
    ...
    _require(
        ...
        "run-003 result fields must match the exact schema",
    )
    _require(
        not _contains_prohibited_evidence(artifact),
        "run-003 result contains prohibited evidence",
    )
    ...
    _require(sum(counts.values()) == 1, "result requires terminal checkpoint")
    expected_status = (
        "completed"
        if counts["completed"]
    ...
    expected_fixed = {
        ...
        "authorization_sha256": _canonical_sha256(authorization),
        "pricing_sha256": pricing_table_sha256(pricing),
        "artifact_paths": RUN_003_ARTIFACT_PATHS,
    }
    ...
    _require(
        ...
        "run-003 result and checkpoint disagree",
    )
    _require(
        ...
        "run-003 result authority changed",
    )
    _require(
        ...
        "run-003 retention policy changed",
    )
    return True


def serialize_run_003_result_artifact(
    artifact: Dict[str, Any],
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
...
def run_003_result_sha256(
    ...
    *,
    ...
    return sha256(
        serialize_run_003_result_artifact(
            artifact,
            authorization=authorization,
            pricing=pricing,
            execution_at_utc=execution_at_utc,
...
def _validate_artifact_path(
    ...
    *,
    ...
    expected = root / RUN_003_ARTIFACT_PATHS[kind]
    _require(
        candidate == expected,
        f"only the exact run-003 {kind} path is allowed",
    )
...
def load_run_003_checkpoint(
    ...
    *,
    ...
    _require(
        ...
        "run-003 checkpoint mode must be 0600",
    )
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        raise ValueError("persisted run-003 checkpoint is malformed") from None
    validate_run_003_checkpoint(
        payload,
        authorization=authorization,
        pricing=pricing,
...
def write_initial_run_003_checkpoint(
    checkpoint_path: str | Path,
    checkpoint: Dict[str, Any],
    *,
    repository_root: str | Path,
    ...
    encoded = serialize_run_003_checkpoint(
        checkpoint,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
...
def replace_run_003_checkpoint_atomic(
    checkpoint_path: str | Path,
    checkpoint: Dict[str, Any],
    *,
    ...
    observed_prior = run_003_checkpoint_sha256(
        prior,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
    ...
    _require(
        ...
        "run-003 checkpoint prior digest mismatch",
    )
    encoded = serialize_run_003_checkpoint(
        checkpoint,
        authorization=authorization,
    ...
    try:
        ...
        os.close(descriptor)
        descriptor = -1
        current = load_run_003_checkpoint(
            path,
            repository_root=repository_root,
            authorization=authorization,
            pricing=pricing,
        ...
        current_sha = run_003_checkpoint_sha256(
            current,
            authorization=authorization,
            pricing=pricing,
            execution_at_utc=execution_at_utc,
        ...
        _require(
            ...
            "run-003 checkpoint changed before replacement",
        )
        ...
        try:
            ...
            pass
        raise
    loaded = load_run_003_checkpoint(
        path,
        repository_root=repository_root,
        authorization=authorization,
        pricing=pricing,
    ...
    _require(loaded == checkpoint, "run-003 checkpoint replacement changed")
    return path


def load_run_003_result_artifact(
    ...
    *,
    ...
    _require(
        stat.S_IMODE(path.stat().st_mode) == 0o600,
        "run-003 result mode must be 0600",
    )
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        raise ValueError("persisted run-003 result is malformed") from None
    validate_run_003_result_artifact(
        payload,
        authorization=authorization,
        pricing=pricing,
...
def write_run_003_result_exclusive(
    result_path: str | Path,
    artifact: Dict[str, Any],
    *,
    repository_root: str | Path,
    ...
    encoded = serialize_run_003_result_artifact(
        artifact,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
