...
SHADOW_AGENTIC_WORKFLOW_STAGE_ORDER = [
    "jd_intelligence",
    "resume_match",
...
def _shadow_chain_has_jd_signals(payload: Dict[str, Any]) -> bool:
    signal_fields = [
        "required_skills",
        "preferred_skills",
        "required_tools",
    ...
    return any(payload.get(field_name) for field_name in signal_fields)


def _shadow_chain_stage_statuses(stages: Dict[str, Dict[str, Any]]) -> Dict[str, str]:
    return {
        "jd_intelligence": _clean_text(
            stages["jd_intelligence"].get("validation_status")
            or stages["jd_intelligence"].get("fallback_used")
            or "unknown"
        ),
        "resume_match": _clean_text(stages["resume_match"].get("match_status")) or "unknown",
        "tailoring_suggestion": _clean_text(stages["tailoring_suggestion"].get("suggestion_status")) or "unknown",
        "critic_guardrail": _clean_text(stages["critic_guardrail"].get("critic_status")) or "unknown",
        "strategy_recommendation": _clean_text(stages["strategy_recommendation"].get("strategy_status")) or "unknown",
    }


def _shadow_chain_stage_called_llm(stages: Dict[str, Dict[str, Any]]) -> bool:
    return any(
        bool((stage.get("safety_metadata") or {}).get("did_call_llm"))
        for stage in stages.values()
        if isinstance(stage, dict)
    )


def _shadow_chain_safety_metadata(stages: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    return {
        "dry_run_only": True,
        "shadow_mode": True,
        "manual_or_service_only": True,
        "did_call_llm": _shadow_chain_stage_called_llm(stages),
        "did_mutate_resume": False,
        "did_mutate_scoring": False,
        "did_change_ranking": False,
        "did_mutate_queue": False,
...
def _shadow_chain_blocking_risks(
    stages: Dict[str, Dict[str, Any]],
    *,
    effective_jd: Dict[str, Any],
) -> List[str]:
    risks: List[str] = []
    strategy = stages.get("strategy_recommendation") or {}
    risks.extend(list(strategy.get("blocking_risks") or []))

    if not _shadow_chain_has_jd_signals(effective_jd):
        risks.append("jd_intelligence_missing")

    resume_match = stages.get("resume_match") or {}
    resume_status = _clean_text(resume_match.get("match_status"))
    if resume_status == "insufficient_evidence":
        risks.append("resume_match_payload_missing")
    elif resume_status == "insufficient_jd_signals":
        risks.append("jd_signals_missing")

    tailoring = stages.get("tailoring_suggestion") or {}
    if _clean_text(tailoring.get("suggestion_status")) == "insufficient_evidence":
        risks.append("tailoring_suggestion_payload_missing")

    critic = stages.get("critic_guardrail") or {}
    if _clean_text(critic.get("critic_status")) == "insufficient_evidence":
        risks.append("critic_guardrail_payload_missing")

    return list(dict.fromkeys(_clean_text(item) for item in risks if _clean_text(item)))


def _shadow_chain_resume_matched_signals(resume_match_stage: Dict[str, Any]) -> set[str]:
    matched: set[str] = set()
    for row in list(resume_match_stage.get("matched_evidence") or []):
        if not isinstance(row, dict):
            continue
        signal = _clean_text(row.get("signal")).lower()
        if signal:
            matched.add(signal)
    return matched


def _shadow_chain_suggestion_matches_resume_evidence(
    suggestion: Dict[str, Any],
    ...
    return any(_clean_text(link.get("signal")).lower() in matched_signals for link in links)


def _shadow_chain_reconcile_tailoring_with_resume_match(
    tailoring_stage: Dict[str, Any],
    resume_match_stage: Dict[str, Any],
) -> Dict[str, Any]:
    matched_signals = _shadow_chain_resume_matched_signals(resume_match_stage)
    if not matched_signals:
        return tailoring_stage

    reconciled = deepcopy(tailoring_stage)
    for field_name in ("guidance_only_suggestions", "rejected_suggestions"):
        reconciled[field_name] = [
            dict(item or {})
            for item in list(reconciled.get(field_name) or [])
            if not (
                isinstance(item, dict)
                and _shadow_chain_suggestion_matches_resume_evidence(item, matched_signals)
            )
        ]

    reconciled["unsupported_claim_risks"] = [
        dict(item or {})
        for item in list(reconciled.get("unsupported_claim_risks") or [])
        if not (
            isinstance(item, dict)
            and _clean_text(item.get("signal")).lower() in matched_signals
        )
    ]
    reconciled["missing_evidence"] = [
        _clean_text(item)
        for item in list(reconciled.get("missing_evidence") or [])
        if not any(f":{signal}:resume_evidence_missing" in _clean_text(item).lower() for signal in matched_signals)
    ]

    if reconciled.get("patch_ready_suggestions"):
        reconciled["suggestion_status"] = "patch_ready_available"
    elif reconciled.get("guidance_only_suggestions"):
        reconciled["suggestion_status"] = "guidance_only"
    elif reconciled.get("rejected_suggestions"):
        reconciled["suggestion_status"] = "rejected_unsupported_claims"
    else:
        reconciled["suggestion_status"] = "insufficient_evidence"
    return reconciled


def build_shadow_agentic_workflow_chain_dry_run_payload(
    *,
    job_title: Any = "",
    company: Any = "",
    location: Any = "",
    ...
    job_id: Any = "",
    jd_adapter: Any = None,
    jd_feature_enabled: bool | None = None,
    tailoring_adapter: Any = None,
    tailoring_feature_enabled: bool | None = None,
    critic_adapter: Any = None,
    critic_feature_enabled: bool | None = None,
    config: Dict[str, Any] | None = None,
) -> Dict[str, Any]:
    """Run the manual dry-run stages in shadow mode without mutating runtime state."""

    source_metadata_copy = deepcopy(source_metadata or {})
    provided_jd = deepcopy(jd_intelligence if jd_intelligence is not None else jd_signals or {})
    resume_variants_copy = deepcopy(resume_variants or [])
    resume_evidence_rows_copy = deepcopy(resume_evidence_rows or [])
    preferences_copy = deepcopy(user_preferences or {})
    config_copy = deepcopy(config or {})
    if not isinstance(source_metadata_copy, dict):
        source_metadata_copy = {}
    if not isinstance(provided_jd, dict):
        provided_jd = {}
    if not isinstance(resume_variants_copy, list):
        resume_variants_copy = []
    if not isinstance(resume_evidence_rows_copy, list):
        resume_evidence_rows_copy = []
    if not isinstance(preferences_copy, dict):
        preferences_copy = {}
    if not isinstance(config_copy, dict):
        config_copy = {}
    effective_resume_variants = (
        resume_variants_copy
        if resume_variants_copy
        else [dict(row or {}) for row in resume_evidence_rows_copy if isinstance(row, dict)]
    )

    jd_stage = build_manual_jd_intelligence_dry_run_payload(
        job_title=job_title,
        company=company,
        location=location,
        job_description=job_description,
        source_metadata=source_metadata_copy,
        context_id=context_id,
        job_id=job_id,
        adapter=jd_adapter,
        feature_enabled=jd_feature_enabled,
        config=config_copy,
    )
    effective_jd = jd_stage if _shadow_chain_has_jd_signals(jd_stage) else provided_jd

    resume_match_stage = build_manual_resume_match_dry_run_payload(
        jd_intelligence=effective_jd,
        resume_variants=effective_resume_variants,
        resume_evidence_rows=resume_evidence_rows_copy,
        selected_resume_id=selected_resume_id,
        context_id=context_id,
        job_id=job_id,
    )
    effective_resume_id = (
        _clean_text(resume_match_stage.get("selected_resume_id"))
        or _clean_text(selected_resume_id)
    )

    tailoring_stage = build_manual_tailoring_suggestion_dry_run_payload(
        jd_intelligence=effective_jd,
        resume_match_payload=resume_match_stage,
        resume_variants=effective_resume_variants,
        resume_evidence_rows=resume_evidence_rows_copy,
        selected_resume_id=effective_resume_id,
        context_id=context_id,
        job_id=job_id,
        adapter=tailoring_adapter,
        feature_enabled=tailoring_feature_enabled,
        config=config_copy,
    )
    tailoring_stage = _shadow_chain_reconcile_tailoring_with_resume_match(
        tailoring_stage,
        resume_match_stage,
    )

    critic_stage = build_manual_critic_guardrail_dry_run_payload(
        tailoring_suggestion_payload=tailoring_stage,
        jd_intelligence=effective_jd,
        resume_variants=effective_resume_variants,
        resume_evidence_rows=resume_evidence_rows_copy,
        context_id=context_id,
        ...
        feature_enabled=critic_feature_enabled,
        config=config_copy,
    )

    strategy_stage = build_manual_strategy_recommendation_dry_run_payload(
        jd_intelligence=effective_jd,
        resume_match_payload=resume_match_stage,
        tailoring_suggestion_payload=tailoring_stage,
        critic_guardrail_payload=critic_stage,
        user_preferences=preferences_copy,
        context_id=context_id,
        job_id=job_id,
    )

    stages = {
        "jd_intelligence": jd_stage,
        "resume_match": resume_match_stage,
        "tailoring_suggestion": tailoring_stage,
        "critic_guardrail": critic_stage,
        "strategy_recommendation": strategy_stage,
    }
    stage_statuses = _shadow_chain_stage_statuses(stages)
    safety_metadata = _shadow_chain_safety_metadata(stages)
    blocking_risks = _shadow_chain_blocking_risks(stages, effective_jd=effective_jd)
    recommendation_action = _clean_text(strategy_stage.get("recommendation_action"))
    if blocking_risks and recommendation_action in {"skip", "save_for_later", ""}:
        recommendation_action = "insufficient_information"
    shadow_chain_status = (
        "completed_with_blockers"
        if blocking_risks or recommendation_action == "insufficient_information"
        else "completed"
    )
    return {
        "shadow_chain_status": shadow_chain_status,
        "dry_run_only": True,
        "stages": stages,
        "stage_order": list(SHADOW_AGENTIC_WORKFLOW_STAGE_ORDER),
        "stage_statuses": stage_statuses,
        "recommendation_action": recommendation_action,
        "blocking_risks": blocking_risks,
        "improvement_actions": list(strategy_stage.get("improvement_actions") or []),
        "required_human_review": bool(strategy_stage.get("required_human_review", True)),
        "confidence": strategy_stage.get("confidence", 0.0),
        "rationale": "Shadow agentic workflow chain composes manual dry-run stage outputs for advisory review only.",
        "context_id": _clean_text(context_id),
        "job_id": _clean_text(job_id),
        "safety_metadata": safety_metadata,
        "manual_surface": True,
        "read_only": True,
        "service_surface": "shadow_agentic_workflow_chain_dry_run",
    }
...
REVIEW_PACKET_PREVIEW_BY_ACTION = {
    "prepare_review_packet": {
        ...
        "sections": [
            {
                ...
                "items": [
                    "Review the shadow recommendation action and rationale.",
                    "Confirm the reviewer decision before any production workflow is considered.",
                ],
            },
            {
                "section_id": "safety_review",
                "heading": "Safety review",
                "items": [
                    "Verify no approval, queue, resume, scoring, ranking, execution, or submission state is changed.",
                    "Keep this packet as preview-only advisory context.",
                ],
            },
        ],
        "checklist": [
            "Confirm the recommendation still needs human review.",
            "Review blocked actions and safety metadata.",
            "Decide whether a real approval workflow should be initiated separately.",
        ],
        "next_safe_step": "review_packet_with_human_confirmation",
    },
    "request_tailoring_revision": {
        "status": "revision_review_preview",
        "title": "Tailoring revision review preview",
        "sections": [
            {
                "section_id": "revision_request",
                "heading": "Revision request",
                "items": [
                    "Review the requested tailoring changes before asking for a new evidence-backed revision.",
                    "Confirm unsupported or missing evidence is not promoted into resume content.",
                ],
            }
        ],
        "checklist": [
            "Review rejected or guidance-only suggestions.",
            "Collect additional resume evidence before any future patch-ready suggestion.",
        ],
        "next_safe_step": "request_evidence_backed_tailoring_revision",
    },
    "save_for_later": {
        "status": "save_for_later_preview",
        "title": "Save-for-later preview",
        "sections": [
            {
                "section_id": "later_review_context",
                "heading": "Later review context",
                "items": [
                    "Keep the recommendation as advisory context only.",
                    "Do not change queue priority or approval state from this preview.",
                ],
            }
        ],
        "checklist": [
            "Revisit when stronger evidence or fresher job context is available.",
            "Keep production state unchanged.",
        ],
        "next_safe_step": "revisit_shadow_recommendation_later",
    },
    "dismiss_recommendation": {
        "status": "dismissal_preview",
        "title": "Dismissal preview",
        "sections": [
            {
                "section_id": "dismissal_context",
                "heading": "Dismissal context",
                "items": [
                    "Preview why the shadow recommendation can be dismissed.",
                    "Do not persist a dismissal or mutate approval state from this dry-run.",
                ],
            }
        ],
        "checklist": [
            "Confirm dismissal is advisory only.",
            "Leave production queue, approval, execution, and submission state unchanged.",
        ],
        "next_safe_step": "no_further_shadow_action",
    },
}


def _review_packet_preview_safety_metadata() -> Dict[str, Any]:
    return {
        "dry_run_only": True,
        "review_packet_preview_only": True,
        "review_only": True,
        "human_confirmation_required": True,
        "did_create_approval": False,
        "did_mutate_approval": False,
...
def _review_packet_stage_summaries(
    action_payload: Dict[str, Any],
    shadow_chain_payload: Dict[str, Any],
) -> Dict[str, Any]:
    stage_statuses = shadow_chain_payload.get("stage_statuses")
    if isinstance(stage_statuses, dict) and stage_statuses:
        return dict(stage_statuses)
    return {
        "action_plan_status": _clean_text(action_payload.get("action_plan_status")) or "missing",
        "source_reviewer_decision": _clean_text(action_payload.get("source_reviewer_decision")),
        "source_recommendation_action": _clean_text(action_payload.get("source_recommendation_action")),
    }


def build_review_packet_preview_dry_run_payload(
    *,
    ...
    """Build a human-readable, non-mutating review packet preview."""

    action_payload = deepcopy(action_plan_payload or {})
    if not isinstance(action_payload, dict):
        action_payload = {}
    shadow_payload = deepcopy(shadow_chain_payload or {})
    if not isinstance(shadow_payload, dict):
        shadow_payload = {}
    if not action_payload:
        action_payload = build_human_approved_action_plan_dry_run_payload(
            decision_capture_payload=deepcopy(decision_capture_payload or {}),
            handoff_payload=deepcopy(handoff_payload or {}),
            shadow_chain_payload=deepcopy(shadow_payload),
            reviewer_decision=reviewer_decision,
            reviewer_note=reviewer_note,
            context_id=context_id,
            job_id=job_id,
    ...
    source_planned_action = _clean_text(action_payload.get("planned_action"))
    blocked_actions = [
        _clean_text(item)
        for item in list(action_payload.get("blocked_actions") or [])
        if _clean_text(item)
    ]
    action_config = REVIEW_PACKET_PREVIEW_BY_ACTION.get(source_planned_action)

    if not source_planned_action:
        review_packet_status = "insufficient_information"
        packet_title = "Review packet unavailable"
        packet_sections: List[Dict[str, Any]] = []
        reviewer_checklist: List[str] = []
        next_safe_step = "collect_valid_action_plan"
        blocked_actions.append("action_plan_missing")
    elif action_config is None:
        review_packet_status = "invalid_action_plan"
        packet_title = "Review packet unavailable"
        packet_sections = []
        reviewer_checklist = []
        next_safe_step = "select_valid_planned_action"
        blocked_actions.append("invalid_planned_action")
    else:
        review_packet_status = str(action_config["status"])
        packet_title = str(action_config["title"])
        packet_sections = deepcopy(list(action_config["sections"]))
        reviewer_checklist = list(action_config["checklist"])
        next_safe_step = str(action_config["next_safe_step"])

    included_stage_summaries = _review_packet_stage_summaries(action_payload, shadow_payload)
    ...
    return {
        ...
        "included_stage_summaries": included_stage_summaries,
        "blocked_actions": list(dict.fromkeys(blocked_actions)),
        "required_human_confirmation": True,
        "next_safe_step": next_safe_step,
        "rationale": (
            "Review packet preview is human-readable advisory context only; it does not create "
            "approvals, mutate queues or resumes, change scoring or ranking, execute, or submit."
        ),
        "context_id": _clean_text(context_id) or _clean_text(action_payload.get("context_id")),
        "job_id": _clean_text(job_id) or _clean_text(action_payload.get("job_id")),
        "safety_metadata": _review_packet_preview_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "review_packet_preview_dry_run",
    }
...
def _guarded_queue_handoff_observability_safety_metadata() -> Dict[str, Any]:
    return {
        "read_only": True,
        "observability_only": True,
        "queue_handoff_audit_only": True,
        "manual_only": True,
        "did_create_approval": False,
        "did_mutate_approval": False,
        "did_update_approval_status": False,
...
def build_guarded_queue_handoff_creation_observability_payload(
    *,
    guarded_queue_handoff_creation_payload: Dict[str, Any] | None = None,
    approval_request_id: Any = "",
    queue_handoff_id: Any = "",
    context_id: Any = "",
    job_id: Any = "",
) -> Dict[str, Any]:
    """Summarize a guarded queue handoff creation result without mutating queue state."""

    source_payload = deepcopy(guarded_queue_handoff_creation_payload or {})
    if not isinstance(source_payload, dict):
        source_payload = {}

    source_status = _clean_text(source_payload.get("queue_handoff_creation_status"))
    request_id = _clean_text(approval_request_id) or _clean_text(source_payload.get("approval_request_id"))
    handoff_id = _clean_text(queue_handoff_id) or _clean_text(source_payload.get("queue_handoff_id"))
    queue_entry_created = bool(source_payload.get("queue_entry_created"))
    blocked_actions = [
        _clean_text(item)
        for item in list(source_payload.get("blocked_actions") or [])
        if _clean_text(item)
    ...
    if not source_payload:
        observability_status = "observed_missing_source"
        handoff_was_created = False
        handoff_was_blocked = True
        blocked_actions.append("guarded_queue_handoff_creation_payload_missing")
        next_safe_step = "run_guarded_queue_handoff_creation_manual_action"
    elif source_status == "created":
        observability_status = "observed_created"
        handoff_was_created = True
        handoff_was_blocked = False
        next_safe_step = "review_created_queue_handoff_before_any_execution"
    elif source_status.startswith("blocked") or source_status == "insufficient_information":
        observability_status = "observed_blocked"
        handoff_was_created = False
        handoff_was_blocked = True
        next_safe_step = _clean_text(source_payload.get("next_safe_step")) or "resolve_guarded_queue_handoff_blockers"
    elif source_status:
        observability_status = "observed_invalid_source"
        handoff_was_created = False
        handoff_was_blocked = True
        blocked_actions.append("guarded_queue_handoff_creation_status_unrecognized")
        next_safe_step = "rebuild_guarded_queue_handoff_creation_payload"
    else:
        observability_status = "insufficient_information"
        handoff_was_created = False
        handoff_was_blocked = True
        blocked_actions.append("guarded_queue_handoff_creation_status_missing")
        next_safe_step = "run_guarded_queue_handoff_creation_manual_action"
    ...
    audit_summary = {
        ...
        "queue_handoff_was_created": handoff_was_created,
        "queue_handoff_was_blocked": handoff_was_blocked,
        "source_mutated_queue": bool(source_safety_payload.get("did_mutate_queue")),
        "source_wrote_queue": bool(source_safety_payload.get("did_write_queue")),
    }
    audit_events = [
        {
            "event_type": "guarded_queue_handoff_observed",
            "approval_request_id": request_id,
            "queue_handoff_id": handoff_id,
            "source_queue_handoff_creation_status": source_status or "missing",
            "queue_handoff_was_created": handoff_was_created,
        }
    ] if source_payload else []
    safety_findings = {
        "source_did_mutate_queue": bool(source_safety_payload.get("did_mutate_queue")),
        "source_did_write_queue": bool(source_safety_payload.get("did_write_queue")),
        "observability_mutated_queue": False,
    ...
    return {
        "queue_handoff_observability_status": observability_status,
        "source_queue_handoff_creation_status": source_status or "missing",
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "queue_entry_created": queue_entry_created,
        ...
        "context_id": _clean_text(context_id) or _clean_text(source_payload.get("context_id")),
        "job_id": _clean_text(job_id) or _clean_text(source_payload.get("job_id")),
        "safety_metadata": _guarded_queue_handoff_observability_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "guarded_queue_handoff_creation_observability",
    }
...
def _execution_launch_gate_preview_safety_metadata() -> Dict[str, Any]:
    return {
        "dry_run_only": True,
        "execution_launch_gate_preview_only": True,
        "manual_only": True,
        "read_only": True,
        "did_create_approval": False,
        "did_mutate_approval": False,
...
def build_execution_launch_gate_preview_payload(
    *,
    approval_request_id: Any = "",
    queue_handoff_id: Any = "",
    execution_readiness_payload: Dict[str, Any] | None = None,
    queue_handoff_observability_payload: Dict[str, Any] | None = None,
    reviewer_confirmation_preview: Any = False,
    context_id: Any = "",
    job_id: Any = "",
) -> Dict[str, Any]:
    """Preview whether future manual execution launch would be allowed."""

    readiness_payload = deepcopy(execution_readiness_payload or {})
    if not isinstance(readiness_payload, dict):
        readiness_payload = {}
    observability_payload = deepcopy(queue_handoff_observability_payload or {})
    if not isinstance(observability_payload, dict):
    ...
    handoff_id = (
        _clean_text(queue_handoff_id)
        or _clean_text(readiness_payload.get("queue_handoff_id"))
        or _clean_text(observability_payload.get("queue_handoff_id"))
    )
    source_readiness_status = _clean_text(readiness_payload.get("execution_readiness_status")) or "missing"
    source_observability_status = (
        _clean_text(observability_payload.get("queue_handoff_observability_status"))
        or "missing"
    )
    ...
    if not request_id:
        gate_status = "blocked_missing_approval_request_id"
        allowed_later = False
        ...
        gate_status = "blocked_missing_queue_handoff_id"
        allowed_later = False
        ready_for_manual_execution = False
        missing_requirements.append("queue_handoff_id")
        blocked_actions.append("queue_handoff_id_missing")
        next_safe_step = "provide_queue_handoff_id"
    elif not readiness_payload or source_readiness_status == "missing":
        gate_status = "blocked_missing_execution_readiness"
        allowed_later = False
        ready_for_manual_execution = False
        missing_requirements.append("execution_readiness_payload")
        blocked_actions.append("execution_readiness_missing")
        next_safe_step = "preview_execution_readiness_before_launch_gate"
    elif source_readiness_status != "ready_for_future_execution":
        gate_status = "blocked_execution_not_ready"
        allowed_later = False
        ready_for_manual_execution = False
        missing_requirements.append("ready_for_future_execution")
        blocked_actions.append("execution_readiness_not_ready")
        next_safe_step = "resolve_execution_readiness_blockers_before_launch_gate"
    elif not observability_payload or source_observability_status == "missing":
        gate_status = "blocked_missing_queue_handoff_observability"
        allowed_later = False
        ready_for_manual_execution = False
        missing_requirements.append("queue_handoff_observability_payload")
        blocked_actions.append("queue_handoff_observability_missing")
        next_safe_step = "view_queue_handoff_audit_before_launch_gate"
    elif source_observability_status != "observed_created":
        gate_status = "blocked_queue_handoff_not_created"
        allowed_later = False
        ready_for_manual_execution = False
        missing_requirements.append("observed_created_queue_handoff")
        blocked_actions.append("queue_handoff_not_observed_created")
        next_safe_step = "resolve_queue_handoff_audit_before_launch_gate"
    else:
        gate_status = "ready_for_future_manual_execution"
        allowed_later = True
        ready_for_manual_execution = True
        next_safe_step = "collect_explicit_future_manual_execution_confirmation"

    return {
        "execution_launch_gate_status": gate_status,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "execution_launch_allowed_later": allowed_later,
        "ready_for_future_manual_execution": ready_for_manual_execution,
        "required_human_confirmation": True,
        "missing_requirements": list(dict.fromkeys(missing_requirements)),
        "blocked_actions": list(dict.fromkeys(blocked_actions)),
        "source_execution_readiness_status": source_readiness_status,
        "source_queue_handoff_observability_status": source_observability_status,
        "next_safe_step": next_safe_step,
        "rationale": (
            "Execution launch gate preview is dry-run only; it does not execute, submit, "
            "mutate queue or approval state, write queue files, change resumes, scoring, "
            "ranking, or add pipeline wiring."
        ),
        "reviewer_confirmation_preview": bool(reviewer_confirmation_preview),
        "context_id": _clean_text(context_id) or _clean_text(readiness_payload.get("context_id")),
        "job_id": _clean_text(job_id) or _clean_text(readiness_payload.get("job_id")),
        "safety_metadata": _execution_launch_gate_preview_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "execution_launch_gate_preview_dry_run",
    }
...
def _guarded_execution_request_creation_safety_metadata(
    *, did_create_execution_request: bool
) -> Dict[str, Any]:
    return {
        "manual_only": True,
        "guarded_execution_request_only": True,
        "human_confirmation_required": True,
        "did_create_approval": False,
        "did_mutate_approval": False,
        "did_update_approval_status": False,
        "did_mutate_queue": bool(did_create_execution_request),
        "did_write_queue": bool(did_create_execution_request),
        "did_create_execution_request": bool(did_create_execution_request),
        "did_execute_application": False,
        "did_submit_application": False,
        "did_mutate_resume": False,
        "did_mutate_scoring": False,
        ...
        "pipeline_wiring_added": False,
        "auto_apply_enabled": False,
        "advisory_only": not bool(did_create_execution_request),
    }


def _manual_execution_request_id(
    *, approval_request_id: str, queue_handoff_id: str, context_id: str, job_id: str
) -> str:
    fingerprint = hashlib.sha256(
        json.dumps(
            {
                "approval_request_id": _clean_text(approval_request_id),
                "queue_handoff_id": _clean_text(queue_handoff_id),
                "context_id": _clean_text(context_id),
                "job_id": _clean_text(job_id),
            },
            sort_keys=True,
    ...
    return f"manual_execution_request_{fingerprint}"


def build_guarded_execution_request_creation_payload(
    *,
    approval_request_id: Any = "",
    queue_handoff_id: Any = "",
    reviewer_confirmation: Any = False,
    execution_request_packet_payload: Dict[str, Any] | None = None,
    execution_launch_gate_payload: Dict[str, Any] | None = None,
    execution_launch_gate_observability_payload: Dict[str, Any] | None = None,
    reviewer_note: Any = "",
    context_id: Any = "",
    job_id: Any = "",
    execution_request_writer: Any = None,
) -> Dict[str, Any]:
    """Create one guarded future execution request artifact without executing anything."""

    packet_payload = deepcopy(execution_request_packet_payload or {})
    if not isinstance(packet_payload, dict):
        packet_payload = {}
    launch_gate = deepcopy(execution_launch_gate_payload or {})
    if not isinstance(launch_gate, dict):
    ...
    if not isinstance(launch_audit, dict):
        launch_audit = {}
    if not packet_payload and any([launch_gate, launch_audit]):
        packet_payload = build_execution_request_packet_preview_payload(
            approval_request_id=approval_request_id,
            queue_handoff_id=queue_handoff_id,
            execution_launch_gate_payload=launch_gate,
            execution_launch_gate_observability_payload=launch_audit,
            reviewer_note=reviewer_note,
            context_id=context_id,
            job_id=job_id,
        )
    ...
    request_id = (
        _clean_text(approval_request_id)
        or _clean_text(packet_payload.get("approval_request_id"))
        or _clean_text(launch_gate.get("approval_request_id"))
        or _clean_text(launch_audit.get("approval_request_id"))
    )
    handoff_id = (
        _clean_text(queue_handoff_id)
        or _clean_text(packet_payload.get("queue_handoff_id"))
        or _clean_text(launch_gate.get("queue_handoff_id"))
        or _clean_text(launch_audit.get("queue_handoff_id"))
    )
    source_packet_status = _clean_text(packet_payload.get("execution_request_packet_status")) or "missing"
    clean_context_id = _clean_text(context_id) or _clean_text(packet_payload.get("context_id"))
    clean_job_id = _clean_text(job_id) or _clean_text(packet_payload.get("job_id"))
    blocked_actions: List[str] = []

    def _blocked(status: str, next_safe_step: str, *blockers: str) -> Dict[str, Any]:
        blocked_actions.extend(blocker for blocker in blockers if blocker)
        return {
            "execution_request_creation_status": status,
            "approval_request_id": request_id,
            "queue_handoff_id": handoff_id,
            "execution_request_id": "",
            "execution_request_created": False,
            "source_execution_request_packet_status": source_packet_status,
            "blocked_actions": list(dict.fromkeys(blocked_actions)),
            "required_human_confirmation": True,
            "next_safe_step": next_safe_step,
            "rationale": (
                "Guarded execution request creation is manual-only and requires a ready "
                "execution request packet plus explicit reviewer confirmation. It creates "
                "no approval records, mutates no approval status, and never executes or submits applications."
            ),
            "reviewer_note": _clean_text(reviewer_note),
            "context_id": clean_context_id,
            "job_id": clean_job_id,
            "safety_metadata": _guarded_execution_request_creation_safety_metadata(
                did_create_execution_request=False
            ),
            "manual_surface": True,
            "service_surface": "guarded_execution_request_creation",
        }

    if reviewer_confirmation is not True:
        return _blocked(
            "blocked_by_missing_confirmation",
            "collect_explicit_execution_request_confirmation",
            "reviewer_confirmation_missing",
        )
    if not request_id:
        return _blocked(
    ...
    if not handoff_id:
        return _blocked(
            "blocked_missing_queue_handoff_id",
            "provide_queue_handoff_id",
            "queue_handoff_id_missing",
        )
    if source_packet_status != "packet_ready_for_human_review":
        return _blocked(
            "blocked_by_packet_preview",
            "preview_ready_execution_request_packet_before_creation",
            "execution_request_packet_not_ready",
        )
    if not callable(execution_request_writer):
        return _blocked(
            "blocked_missing_execution_request_writer",
            "configure_existing_execution_request_writer_before_manual_creation",
            "execution_request_writer_unavailable",
        )

    execution_request_id = _manual_execution_request_id(
        approval_request_id=request_id,
        queue_handoff_id=handoff_id,
        context_id=clean_context_id,
        job_id=clean_job_id,
    )
    execution_request_entry = {
        ...
        "job_id": clean_job_id,
        "reviewer_note": _clean_text(reviewer_note),
        "manual_guarded_execution_request": True,
        "execute_application": False,
        "submit_application": False,
        "pipeline_wiring_added": False,
    }
    try:
        writer_result = execution_request_writer(deepcopy(execution_request_entry))
    except Exception:
        return _blocked(
            "blocked_by_storage_error",
            "retry_guarded_execution_request_creation",
            "execution_request_writer_error",
        )
    ...
    return {
        ...
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "execution_request_id": _clean_text(
            writer_result.get("execution_request_id") if isinstance(writer_result, dict) else ""
        ) or execution_request_id,
        ...
        "rationale": (
            "Created exactly one guarded execution request/control artifact through the "
            "injected existing writer; no application execution, submission, approval status, "
            "resume, scoring, ranking, or pipeline state was changed."
        ),
        "reviewer_note": _clean_text(reviewer_note),
        "context_id": clean_context_id,
        "job_id": clean_job_id,
        ...
        "manual_surface": True,
        "service_surface": "guarded_execution_request_creation",
        "execution_request_writer_result": deepcopy(writer_result) if isinstance(writer_result, dict) else {},
    }


def _guarded_execution_request_observability_safety_metadata() -> Dict[str, Any]:
    return {
        "read_only": True,
        "observability_only": True,
        "execution_request_audit_only": True,
        "manual_only": True,
        "did_create_approval": False,
        "did_mutate_approval": False,
        "did_update_approval_status": False,
...
def build_guarded_execution_request_creation_observability_payload(
    *,
    ...
    """Summarize a guarded execution request creation result without creating anything."""

    source_payload = deepcopy(guarded_execution_request_creation_payload or {})
    if not isinstance(source_payload, dict):
        source_payload = {}

    source_status = _clean_text(source_payload.get("execution_request_creation_status"))
    request_id = _clean_text(approval_request_id) or _clean_text(source_payload.get("approval_request_id"))
    handoff_id = _clean_text(queue_handoff_id) or _clean_text(source_payload.get("queue_handoff_id"))
    execution_id = (
        _clean_text(execution_request_id)
        or _clean_text(source_payload.get("execution_request_id"))
    )
    execution_request_created = bool(source_payload.get("execution_request_created"))
    blocked_actions = [
        _clean_text(item)
        for item in list(source_payload.get("blocked_actions") or [])
        if _clean_text(item)
    ...
    if not source_payload:
        ...
        request_was_blocked = True
        blocked_actions.append("guarded_execution_request_creation_payload_missing")
        next_safe_step = "run_guarded_execution_request_creation_manual_action"
    elif source_status == "created":
        observability_status = "observed_created"
        request_was_created = True
        request_was_blocked = False
        next_safe_step = "review_created_execution_request_before_any_future_execution"
    elif source_status.startswith("blocked") or source_status == "insufficient_information":
        observability_status = "observed_blocked"
        request_was_created = False
        request_was_blocked = True
        next_safe_step = _clean_text(source_payload.get("next_safe_step")) or "resolve_guarded_execution_request_blockers"
    elif source_status:
        observability_status = "observed_invalid_source"
        request_was_created = False
        request_was_blocked = True
        blocked_actions.append("guarded_execution_request_creation_status_unrecognized")
        next_safe_step = "rebuild_guarded_execution_request_creation_payload"
    else:
        observability_status = "insufficient_information"
        request_was_created = False
        request_was_blocked = True
        blocked_actions.append("guarded_execution_request_creation_status_missing")
        next_safe_step = "run_guarded_execution_request_creation_manual_action"
    ...
    audit_summary = {
        ...
        "execution_request_was_created": request_was_created,
        "execution_request_was_blocked": request_was_blocked,
        "source_created_execution_request": bool(source_safety_payload.get("did_create_execution_request")),
        "source_mutated_queue": bool(source_safety_payload.get("did_mutate_queue")),
        "source_wrote_queue": bool(source_safety_payload.get("did_write_queue")),
    }
    audit_events = [
        {
            "event_type": "guarded_execution_request_observed",
            "approval_request_id": request_id,
            "queue_handoff_id": handoff_id,
            "execution_request_id": execution_id,
            "source_execution_request_creation_status": source_status or "missing",
            "execution_request_was_created": request_was_created,
        }
    ] if source_payload else []
    safety_findings = {
        "source_did_create_execution_request": bool(source_safety_payload.get("did_create_execution_request")),
        "source_did_mutate_queue": bool(source_safety_payload.get("did_mutate_queue")),
        "source_did_write_queue": bool(source_safety_payload.get("did_write_queue")),
    ...
    return {
        "execution_request_observability_status": observability_status,
        "source_execution_request_creation_status": source_status or "missing",
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "execution_request_id": execution_id,
        ...
        "context_id": _clean_text(context_id) or _clean_text(source_payload.get("context_id")),
        "job_id": _clean_text(job_id) or _clean_text(source_payload.get("job_id")),
        "safety_metadata": _guarded_execution_request_observability_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "guarded_execution_request_creation_observability",
    }
...
def _guarded_execution_request_status_transition_safety_metadata(
    *, did_update_execution_request_status: bool
) -> Dict[str, Any]:
    return {
        "manual_only": True,
        "guarded_execution_request_status_transition_only": True,
        "human_confirmation_required": True,
        "did_create_execution_request": False,
        "did_update_execution_request_status": bool(did_update_execution_request_status),
        "did_create_approval": False,
        "did_mutate_approval": False,
        "did_update_approval_status": False,
        "did_mutate_queue": bool(did_update_execution_request_status),
        "did_write_queue": bool(did_update_execution_request_status),
        "did_execute_application": False,
        "did_submit_application": False,
        "did_mutate_resume": False,
        "did_mutate_scoring": False,
        ...
        "pipeline_wiring_added": False,
        "auto_apply_enabled": False,
        "advisory_only": not bool(did_update_execution_request_status),
    }


def build_guarded_execution_request_status_transition_payload(
    *,
    execution_request_id: Any = "",
    requested_transition: Any = "",
    reviewer_confirmation: Any = False,
    execution_request_status_transition_preview_payload: Dict[str, Any] | None = None,
    execution_request_readback_payload: Dict[str, Any] | None = None,
    approval_request_id: Any = "",
    queue_handoff_id: Any = "",
    reviewer_note: Any = "",
    context_id: Any = "",
    job_id: Any = "",
    execution_request_status_writer: Any = None,
) -> Dict[str, Any]:
    """Apply a guarded execution request status transition without executing anything."""

    preview_payload = deepcopy(execution_request_status_transition_preview_payload or {})
    if not isinstance(preview_payload, dict):
        preview_payload = {}
    readback_payload = deepcopy(execution_request_readback_payload or {})
    if not isinstance(readback_payload, dict):
    ...
    transition = _clean_text(requested_transition) or _clean_text(preview_payload.get("requested_transition"))
    source_preview_status = (
        _clean_text(preview_payload.get("execution_request_status_transition_preview_status"))
        or "missing"
    )
    previous_status = (
        _clean_text(preview_payload.get("previous_execution_request_status"))
        or _clean_text(readback_payload.get("execution_request_status"))
    )
    proposed_status = _clean_text(preview_payload.get("proposed_execution_request_status"))
    if not proposed_status:
        proposed_status = _clean_text(
            EXECUTION_REQUEST_STATUS_TRANSITION_PREVIEW_OPTIONS.get(transition, {}).get("would_change_status_to")
        )
    blocked_actions: List[str] = []
    ...
    def _blocked(status: str, next_safe_step: str, *blockers: str) -> Dict[str, Any]:
        blocked_actions.extend(blocker for blocker in blockers if blocker)
        return {
            "execution_request_status_transition_status": status,
            "execution_request_id": execution_id,
            "approval_request_id": request_id,
            "queue_handoff_id": handoff_id,
            "requested_transition": transition,
            "previous_execution_request_status": previous_status,
            "new_execution_request_status": previous_status,
            ...
            "next_safe_step": next_safe_step,
            "rationale": (
                "Guarded execution request status transition is manual-only and requires a "
                "ready transition preview plus explicit reviewer confirmation. It does not "
                "execute, submit, mutate approvals, resumes, scoring, ranking, or pipeline wiring."
            ),
            "reviewer_note": _clean_text(reviewer_note),
            "context_id": _clean_text(context_id) or _clean_text(preview_payload.get("context_id")),
            "job_id": _clean_text(job_id) or _clean_text(preview_payload.get("job_id")),
            "safety_metadata": _guarded_execution_request_status_transition_safety_metadata(
                did_update_execution_request_status=False
            ),
            "manual_surface": True,
            "service_surface": "guarded_execution_request_status_transition",
        }

    if reviewer_confirmation is not True:
        return _blocked(
            "blocked_by_missing_confirmation",
            "collect_explicit_execution_request_status_transition_confirmation",
            "reviewer_confirmation_missing",
        )
    if not execution_id:
        return _blocked(
            "blocked_missing_execution_request_id",
            "provide_execution_request_id",
            "execution_request_id_missing",
        )
    if not transition:
        return _blocked(
            "blocked_missing_requested_transition",
            "select_supported_execution_request_transition",
            "requested_transition_missing",
        )
    if transition not in EXECUTION_REQUEST_STATUS_TRANSITION_PREVIEW_OPTIONS:
        return _blocked(
    ...
    if source_preview_status != "ready_for_future_status_transition":
        return _blocked(
            ...
            "run_execution_request_status_transition_preview_before_apply",
            "execution_request_status_transition_preview_not_ready",
        )
    expected_status = _clean_text(
        EXECUTION_REQUEST_STATUS_TRANSITION_PREVIEW_OPTIONS[transition]["would_change_status_to"]
    )
    if proposed_status != expected_status:
        return _blocked(
            "blocked_by_transition_preview",
            "rebuild_execution_request_status_transition_preview",
            "proposed_execution_request_status_mismatch",
        )
    if not callable(execution_request_status_writer):
        return _blocked(
            "blocked_missing_execution_request_status_writer",
            "configure_existing_execution_request_status_writer_before_manual_transition",
            "execution_request_status_writer_unavailable",
        )
    ...
    update_payload = {
        ...
        "requested_transition": transition,
        "previous_execution_request_status": previous_status,
        "new_execution_request_status": expected_status,
        "reviewer_note": _clean_text(reviewer_note),
        "manual_guarded_execution_request_status_transition": True,
        "pipeline_wiring_added": False,
    }
    try:
        writer_result = execution_request_status_writer(deepcopy(update_payload))
    except Exception:
        return _blocked(
            "blocked_by_storage_error",
            "retry_guarded_execution_request_status_transition",
            "execution_request_status_writer_error",
        )

    new_status = (
        _clean_text(writer_result.get("new_execution_request_status") if isinstance(writer_result, dict) else "")
        or _clean_text(writer_result.get("execution_request_status") if isinstance(writer_result, dict) else "")
        or expected_status
    )
    return {
        ...
        "requested_transition": transition,
        "previous_execution_request_status": previous_status,
        "new_execution_request_status": new_status,
        ...
        "rationale": (
            "Explicit reviewer confirmation was present and the transition preview was ready; "
            "only the execution request/control artifact status was updated through the injected writer."
        ),
        "reviewer_note": _clean_text(reviewer_note),
        "context_id": _clean_text(context_id) or _clean_text(preview_payload.get("context_id")),
        "job_id": _clean_text(job_id) or _clean_text(preview_payload.get("job_id")),
        ...
        "manual_surface": True,
        "service_surface": "guarded_execution_request_status_transition",
        "execution_request_status_writer_result": deepcopy(writer_result) if isinstance(writer_result, dict) else {},
    }


def _guarded_execution_request_status_transition_observability_safety_metadata() -> Dict[str, Any]:
    return {
        "read_only": True,
        "observability_only": True,
        "execution_request_status_transition_audit_only": True,
        "manual_only": True,
        "did_create_execution_request": False,
        "did_update_execution_request_status": False,
        "did_create_approval": False,
...
def build_guarded_execution_request_status_transition_observability_payload(
    *,
    guarded_execution_request_status_transition_payload: Dict[str, Any] | None = None,
    execution_request_id: Any = "",
    approval_request_id: Any = "",
    queue_handoff_id: Any = "",
    context_id: Any = "",
    ...
    """Summarize a guarded execution request status transition result without mutating anything."""

    source_payload = deepcopy(guarded_execution_request_status_transition_payload or {})
    if not isinstance(source_payload, dict):
        source_payload = {}

    source_status = _clean_text(source_payload.get("execution_request_status_transition_status"))
    execution_id = _clean_text(execution_request_id) or _clean_text(source_payload.get("execution_request_id"))
    request_id = _clean_text(approval_request_id) or _clean_text(source_payload.get("approval_request_id"))
    handoff_id = _clean_text(queue_handoff_id) or _clean_text(source_payload.get("queue_handoff_id"))
    transition = _clean_text(source_payload.get("requested_transition"))
    previous_status = _clean_text(source_payload.get("previous_execution_request_status"))
    new_status = _clean_text(source_payload.get("new_execution_request_status"))
    status_updated = bool(source_payload.get("execution_request_status_updated"))
    blocked_actions = [
        _clean_text(item)
        for item in list(source_payload.get("blocked_actions") or [])
        if _clean_text(item)
    ...
    if not source_payload:
        ...
        transition_was_applied = False
        transition_was_blocked = True
        blocked_actions.append("guarded_execution_request_status_transition_payload_missing")
        next_safe_step = "run_guarded_execution_request_status_transition_manual_action"
    elif source_status == "updated":
        observability_status = "observed_updated"
        transition_was_applied = True
        transition_was_blocked = False
        ...
        transition_was_blocked = True
        next_safe_step = (
            _clean_text(source_payload.get("next_safe_step"))
            or "resolve_guarded_execution_request_status_transition_blockers"
        )
    elif source_status:
        observability_status = "observed_invalid_source"
        transition_was_applied = False
        transition_was_blocked = True
        blocked_actions.append("guarded_execution_request_status_transition_status_unrecognized")
        next_safe_step = "rebuild_guarded_execution_request_status_transition_payload"
    else:
        observability_status = "insufficient_information"
        transition_was_applied = False
        transition_was_blocked = True
        blocked_actions.append("guarded_execution_request_status_transition_status_missing")
        next_safe_step = "run_guarded_execution_request_status_transition_manual_action"
    ...
    audit_summary = {
        ...
        "previous_execution_request_status": previous_status,
        "new_execution_request_status": new_status,
        "execution_request_status_updated": status_updated,
        "transition_was_applied": transition_was_applied,
        "transition_was_blocked": transition_was_blocked,
        "source_updated_execution_request_status": bool(
            source_safety_payload.get("did_update_execution_request_status")
        ),
    }
    audit_events = [
        {
            "event_type": "guarded_execution_request_status_transition_observed",
            "execution_request_id": execution_id,
            "source_transition_status": source_status or "missing",
            "transition_was_applied": transition_was_applied,
        }
    ...
    safety_findings = {
        "source_did_update_execution_request_status": bool(
            source_safety_payload.get("did_update_execution_request_status")
        ),
    ...
    return {
        "execution_request_status_transition_observability_status": observability_status,
        "source_execution_request_status_transition_status": source_status or "missing",
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "requested_transition": transition,
        ...
        "new_execution_request_status": new_status,
        "execution_request_status_updated": status_updated,
        "execution_request_status_transition_was_blocked": transition_was_blocked,
        "execution_request_status_transition_was_applied": transition_was_applied,
        "audit_summary": audit_summary,
        "audit_events": audit_events,
        "safety_findings": safety_findings,
        "blocked_actions": list(dict.fromkeys(blocked_actions)),
        ...
        "context_id": _clean_text(context_id) or _clean_text(source_payload.get("context_id")),
        "job_id": _clean_text(job_id) or _clean_text(source_payload.get("job_id")),
        "safety_metadata": _guarded_execution_request_status_transition_observability_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "guarded_execution_request_status_transition_observability",
    }
...
def _application_execution_simulation_observability_safety_metadata() -> Dict[str, Any]:
    return {
        "read_only": True,
        "observability_only": True,
        "application_execution_simulation_audit_only": True,
        "manual_only": True,
        "did_create_execution_request": False,
        "did_update_execution_request_status": False,
        "did_create_approval": False,
...
def build_application_execution_simulation_observability_payload(
    *,
    ...
    """Summarize an application execution simulation preview without launching anything."""

    source_payload = deepcopy(application_execution_simulation_payload or {})
    if not isinstance(source_payload, dict):
        source_payload = {}

    source_status = _clean_text(source_payload.get("application_execution_simulation_status"))
    execution_id = _clean_text(execution_request_id) or _clean_text(source_payload.get("execution_request_id"))
    request_id = _clean_text(approval_request_id) or _clean_text(source_payload.get("approval_request_id"))
    handoff_id = _clean_text(queue_handoff_id) or _clean_text(source_payload.get("queue_handoff_id"))
    simulated_allowed = bool(source_payload.get("simulated_execution_allowed_later"))
    blocked_actions = [
        _clean_text(item)
        for item in list(source_payload.get("blocked_actions") or [])
        if _clean_text(item)
    ...
    if not source_payload:
        observability_status = "observed_missing_source"
        simulation_was_ready = False
        simulation_was_blocked = True
        blocked_actions.append("application_execution_simulation_payload_missing")
        next_safe_step = "run_application_execution_simulation_preview"
    elif source_status == "simulation_ready":
        observability_status = "observed_ready"
        ...
        simulation_was_ready = False
        simulation_was_blocked = True
        next_safe_step = _clean_text(source_payload.get("next_safe_step")) or "resolve_execution_simulation_blockers"
    elif source_status:
        observability_status = "observed_invalid_source"
        simulation_was_ready = False
        simulation_was_blocked = True
        blocked_actions.append("application_execution_simulation_status_unrecognized")
        next_safe_step = "rebuild_application_execution_simulation_payload"
    else:
        observability_status = "insufficient_information"
        simulation_was_ready = False
        simulation_was_blocked = True
        blocked_actions.append("application_execution_simulation_status_missing")
        next_safe_step = "run_application_execution_simulation_preview"
    ...
    audit_summary = {
        ...
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "simulation_was_ready": simulation_was_ready,
        "simulation_was_blocked": simulation_was_blocked,
        "simulated_execution_allowed_later": simulated_allowed,
        "source_executed_application": bool(source_safety_payload.get("did_execute_application")),
        "source_submitted_application": bool(source_safety_payload.get("did_submit_application")),
    }
    audit_events = [
        {
            "event_type": "application_execution_simulation_observed",
            "execution_request_id": execution_id,
            "source_simulation_status": source_status or "missing",
            "simulation_was_ready": simulation_was_ready,
        }
    ] if source_payload else []
    safety_findings = {
    ...
    return {
        "application_execution_simulation_observability_status": observability_status,
        "source_application_execution_simulation_status": source_status or "missing",
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "simulation_was_ready": simulation_was_ready,
        ...
        "context_id": _clean_text(context_id) or _clean_text(source_payload.get("context_id")),
        "job_id": _clean_text(job_id) or _clean_text(source_payload.get("job_id")),
        "safety_metadata": _application_execution_simulation_observability_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "application_execution_simulation_observability",
    }
...
def _guarded_application_execution_launch_request_safety_metadata(
    *, did_create_execution_launch_request: bool
) -> Dict[str, Any]:
    return {
        "manual_only": True,
        "guarded_application_execution_launch_request_only": True,
        "human_confirmation_required": True,
        "did_create_execution_launch_request": bool(did_create_execution_launch_request),
        "did_create_execution_request": False,
        "did_update_execution_request_status": False,
        "did_create_approval": False,
        "did_mutate_approval": False,
        "did_update_approval_status": False,
        "did_mutate_queue": bool(did_create_execution_launch_request),
        "did_write_queue": bool(did_create_execution_launch_request),
        "did_execute_application": False,
        "did_submit_application": False,
        "did_mutate_resume": False,
        "did_mutate_scoring": False,
        ...
        "pipeline_wiring_added": False,
        "auto_apply_enabled": False,
        "advisory_only": not bool(did_create_execution_launch_request),
    }


def _manual_execution_launch_request_id(
    *,
    ...
    fingerprint = hashlib.sha256(
        json.dumps(
            {
                "execution_request_id": _clean_text(execution_request_id),
                "approval_request_id": _clean_text(approval_request_id),
                "queue_handoff_id": _clean_text(queue_handoff_id),
                "context_id": _clean_text(context_id),
                "job_id": _clean_text(job_id),
    ...
    return f"manual_execution_launch_request_{fingerprint}"


def build_guarded_application_execution_launch_request_payload(
    *,
    execution_request_id: Any = "",
    approval_request_id: Any = "",
    queue_handoff_id: Any = "",
    reviewer_confirmation: Any = False,
    application_execution_preflight_payload: Dict[str, Any] | None = None,
    application_execution_preflight_observability_payload: Dict[str, Any] | None = None,
    application_execution_simulation_payload: Dict[str, Any] | None = None,
    application_execution_simulation_observability_payload: Dict[str, Any] | None = None,
    reviewer_note: Any = "",
    context_id: Any = "",
    job_id: Any = "",
    execution_launch_request_writer: Any = None,
) -> Dict[str, Any]:
    """Create one guarded future execution launch request artifact without executing anything."""

    preflight_payload = deepcopy(application_execution_preflight_payload or {})
    if not isinstance(preflight_payload, dict):
        preflight_payload = {}
    preflight_observability_payload = deepcopy(application_execution_preflight_observability_payload or {})
    if not isinstance(preflight_observability_payload, dict):
        preflight_observability_payload = {}
    simulation_payload = deepcopy(application_execution_simulation_payload or {})
    if not isinstance(simulation_payload, dict):
        simulation_payload = {}
    simulation_observability_payload = deepcopy(application_execution_simulation_observability_payload or {})
    ...
    execution_id = (
        _clean_text(execution_request_id)
        or _clean_text(preflight_payload.get("execution_request_id"))
        or _clean_text(preflight_observability_payload.get("execution_request_id"))
        or _clean_text(simulation_payload.get("execution_request_id"))
        or _clean_text(simulation_observability_payload.get("execution_request_id"))
    )
    request_id = (
        _clean_text(approval_request_id)
        or _clean_text(preflight_payload.get("approval_request_id"))
        or _clean_text(preflight_observability_payload.get("approval_request_id"))
        or _clean_text(simulation_payload.get("approval_request_id"))
        or _clean_text(simulation_observability_payload.get("approval_request_id"))
    )
    handoff_id = (
        _clean_text(queue_handoff_id)
        or _clean_text(preflight_payload.get("queue_handoff_id"))
        or _clean_text(preflight_observability_payload.get("queue_handoff_id"))
        or _clean_text(simulation_payload.get("queue_handoff_id"))
        or _clean_text(simulation_observability_payload.get("queue_handoff_id"))
    )
    source_preflight_status = (
        _clean_text(preflight_payload.get("application_execution_preflight_status"))
        or "missing"
    )
    source_preflight_observability_status = (
        _clean_text(
            preflight_observability_payload.get("application_execution_preflight_observability_status")
        )
        or "missing"
    )
    clean_context_id = (
        _clean_text(context_id)
        or _clean_text(preflight_payload.get("context_id"))
        or _clean_text(preflight_observability_payload.get("context_id"))
    )
    clean_job_id = (
        _clean_text(job_id)
        or _clean_text(preflight_payload.get("job_id"))
        or _clean_text(preflight_observability_payload.get("job_id"))
    )
    blocked_actions: List[str] = []
    ...
    def _blocked(status: str, next_safe_step: str, *blockers: str) -> Dict[str, Any]:
        blocked_actions.extend(blocker for blocker in blockers if blocker)
        return {
            "application_execution_launch_request_status": status,
            "execution_request_id": execution_id,
            "approval_request_id": request_id,
            "queue_handoff_id": handoff_id,
            "execution_launch_request_id": "",
            "execution_launch_request_created": False,
            "source_application_execution_preflight_status": source_preflight_status,
            "source_application_execution_preflight_observability_status": (
                source_preflight_observability_status
            ),
            "blocked_actions": list(dict.fromkeys(blocked_actions)),
            "required_human_confirmation": True,
            "next_safe_step": next_safe_step,
            "rationale": (
                "Guarded application execution launch request creation is manual-only and requires "
                "ready preflight evidence plus explicit reviewer confirmation. It creates no execution "
                "request, updates no execution status, mutates no approvals, and never executes or submits."
            ),
            "reviewer_note": _clean_text(reviewer_note),
            "context_id": clean_context_id,
            "job_id": clean_job_id,
            "safety_metadata": _guarded_application_execution_launch_request_safety_metadata(
                did_create_execution_launch_request=False
            ),
            "manual_surface": True,
            "service_surface": "guarded_application_execution_launch_request",
        }

    if reviewer_confirmation is not True:
        return _blocked(
            "blocked_by_missing_confirmation",
            "collect_explicit_execution_launch_request_confirmation",
            "reviewer_confirmation_missing",
        )
    if not execution_id:
        return _blocked(
    ...
    if source_preflight_status != "preflight_ready_for_human_review":
        return _blocked(
            "blocked_by_preflight",
            "run_ready_application_execution_preflight_checklist_before_launch_request",
            "application_execution_preflight_not_ready",
        )
    if source_preflight_observability_status != "observed_ready":
        return _blocked(
            "blocked_missing_preflight_observability",
            "run_application_execution_preflight_observability_before_launch_request",
            "application_execution_preflight_observability_not_ready",
        )
    if not callable(execution_launch_request_writer):
        return _blocked(
            "blocked_missing_execution_launch_request_writer",
            "configure_existing_execution_launch_request_writer_before_manual_creation",
            "execution_launch_request_writer_unavailable",
        )

    launch_request_id = _manual_execution_launch_request_id(
        execution_request_id=execution_id,
        approval_request_id=request_id,
        queue_handoff_id=handoff_id,
        context_id=clean_context_id,
    ...
    launch_request_entry = {
        ...
        "job_id": clean_job_id,
        "reviewer_note": _clean_text(reviewer_note),
        "manual_guarded_application_execution_launch_request": True,
        "execute_application": False,
        "submit_application": False,
        "pipeline_wiring_added": False,
    }
    try:
        writer_result = execution_launch_request_writer(deepcopy(launch_request_entry))
    except Exception:
        return _blocked(
            "blocked_by_storage_error",
            "retry_guarded_execution_launch_request_creation",
            "execution_launch_request_writer_error",
        )

    return {
        "application_execution_launch_request_status": "created",
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        ...
        "execution_launch_request_created": True,
        "source_application_execution_preflight_status": source_preflight_status,
        "source_application_execution_preflight_observability_status": (
            source_preflight_observability_status
        ),
        "blocked_actions": [],
        "required_human_confirmation": True,
        "next_safe_step": "review_created_execution_launch_request_before_any_execution",
        "rationale": (
            "Created exactly one guarded application execution launch request/control artifact "
            "through the injected existing writer; no application execution, submission, execution "
            "request status, approval status, resume, scoring, ranking, or pipeline state was changed."
        ),
        "reviewer_note": _clean_text(reviewer_note),
        "context_id": clean_context_id,
        "job_id": clean_job_id,
        "safety_metadata": _guarded_application_execution_launch_request_safety_metadata(
            did_create_execution_launch_request=True
        ),
        "manual_surface": True,
        "service_surface": "guarded_application_execution_launch_request",
        "execution_launch_request_writer_result": (
            deepcopy(writer_result) if isinstance(writer_result, dict) else {}
        ),
    }


def _guarded_application_execution_launch_request_observability_safety_metadata() -> Dict[str, Any]:
    return {
        "read_only": True,
        "observability_only": True,
        "application_execution_launch_request_audit_only": True,
        "manual_only": True,
        "did_create_execution_launch_request": False,
        "did_create_execution_request": False,
        "did_update_execution_request_status": False,
...
def build_guarded_application_execution_launch_request_observability_payload(
    *,
    ...
    """Summarize a guarded execution launch request result without creating anything."""

    source_payload = deepcopy(guarded_application_execution_launch_request_payload or {})
    if not isinstance(source_payload, dict):
        source_payload = {}

    source_status = _clean_text(source_payload.get("application_execution_launch_request_status"))
    launch_request_id = _clean_text(execution_launch_request_id) or _clean_text(
        source_payload.get("execution_launch_request_id")
    )
    execution_id = _clean_text(execution_request_id) or _clean_text(
        source_payload.get("execution_request_id")
    )
    request_id = _clean_text(approval_request_id) or _clean_text(
        source_payload.get("approval_request_id")
    )
    handoff_id = _clean_text(queue_handoff_id) or _clean_text(source_payload.get("queue_handoff_id"))
    clean_context_id = _clean_text(context_id) or _clean_text(source_payload.get("context_id"))
    clean_job_id = _clean_text(job_id) or _clean_text(source_payload.get("job_id"))
    source_safety = source_payload.get("safety_metadata")
    if not isinstance(source_safety, dict):
        source_safety = {}
    source_blocked_actions = source_payload.get("blocked_actions")
    blocked_actions = (
        [str(action) for action in source_blocked_actions if str(action).strip()]
        if isinstance(source_blocked_actions, list)
        else []
    )

    if not source_payload:
        observability_status = "observed_missing_source"
        blocked_actions.append("guarded_application_execution_launch_request_payload_missing")
        next_safe_step = "run_guarded_application_execution_launch_request_manual_action_first"
    elif not source_status:
        observability_status = "insufficient_information"
        blocked_actions.append("application_execution_launch_request_status_missing")
        next_safe_step = "provide_guarded_application_execution_launch_request_payload"
    elif source_status == "created":
        observability_status = "observed_created"
        next_safe_step = (
            _clean_text(source_payload.get("next_safe_step"))
            or "review_created_execution_launch_request_before_any_execution"
        )
    elif source_status.startswith("blocked"):
        observability_status = "observed_blocked"
        next_safe_step = (
            _clean_text(source_payload.get("next_safe_step"))
            or "resolve_guarded_execution_launch_request_blockers"
        )
    else:
        observability_status = "observed_invalid_source"
        blocked_actions.append("application_execution_launch_request_status_unrecognized")
        next_safe_step = "rebuild_guarded_application_execution_launch_request_payload"

    launch_request_created = bool(source_payload.get("execution_launch_request_created")) and (
        source_status == "created"
    )
    launch_request_blocked = observability_status in {
        "observed_blocked",
    ...
    audit_summary = {
        "source_application_execution_launch_request_status": source_status or "missing",
        "execution_launch_request_id": launch_request_id,
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "source_created_execution_launch_request": bool(
            source_safety.get("did_create_execution_launch_request")
        ),
        "source_created_execution_request": bool(source_safety.get("did_create_execution_request")),
        "source_updated_execution_request_status": bool(
            source_safety.get("did_update_execution_request_status")
        ),
        "source_mutated_queue": bool(source_safety.get("did_mutate_queue")),
        "source_wrote_queue": bool(source_safety.get("did_write_queue")),
        "source_executed_application": bool(source_safety.get("did_execute_application")),
        "source_submitted_application": bool(source_safety.get("did_submit_application")),
    }
    audit_events = []
    if source_payload:
        audit_events.append(
            {
                "event_type": "guarded_application_execution_launch_request_observed",
                "source_application_execution_launch_request_status": source_status or "missing",
                "execution_launch_request_id": launch_request_id,
                "execution_request_id": execution_id,
                "approval_request_id": request_id,
                "queue_handoff_id": handoff_id,
                "execution_launch_request_created": launch_request_created,
                "execution_launch_request_was_blocked": launch_request_blocked,
            }
        )
    safety_findings = {
        "observability_created_execution_launch_request": False,
        "observability_created_execution_request": False,
        "observability_updated_execution_request_status": False,
        "observability_mutated_queue": False,
        "observability_wrote_queue": False,
    ...
    return {
        "application_execution_launch_request_observability_status": observability_status,
        "source_application_execution_launch_request_status": source_status or "missing",
        "execution_launch_request_id": launch_request_id,
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        ...
        "execution_launch_request_created": launch_request_created,
        "execution_launch_request_was_blocked": launch_request_blocked,
        "execution_launch_request_was_created": launch_request_created,
        "audit_summary": audit_summary,
        "audit_events": audit_events,
        "safety_findings": safety_findings,
        "blocked_actions": list(dict.fromkeys(blocked_actions)),
        ...
        "job_id": clean_job_id,
        "safety_metadata": (
            _guarded_application_execution_launch_request_observability_safety_metadata()
        ),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "guarded_application_execution_launch_request_observability",
    }


def _application_execution_launch_request_readback_safety_metadata() -> Dict[str, Any]:
    return {
        "read_only": True,
        "application_execution_launch_request_readback_only": True,
        "manual_only": True,
        "did_create_execution_launch_request": False,
        "did_create_execution_request": False,
        "did_update_execution_request_status": False,
...
def build_application_execution_launch_request_readback_payload(
    *,
    execution_launch_request_id: Any = "",
    execution_request_id: Any = "",
    approval_request_id: Any = "",
    ...
    """Build a read-only normalized detail view from provided launch request evidence."""
    ...
    source_status = _clean_text(creation_payload.get("application_execution_launch_request_status"))
    source_observability_status = _clean_text(
        observability_payload.get("application_execution_launch_request_observability_status")
    )
    source_observed_status = _clean_text(
        observability_payload.get("source_application_execution_launch_request_status")
    )
    if not source_status and source_observed_status:
        source_status = source_observed_status

    missing_requirements: List[str] = []
    blocked_actions: List[str] = []
    for source in (creation_payload, observability_payload):
        source_blockers = source.get("blocked_actions")
        if isinstance(source_blockers, list):
            blocked_actions.extend(str(action) for action in source_blockers if str(action).strip())

    source_available = bool(creation_payload or observability_payload)
    creation_found = (
        source_status == "created"
        and bool(creation_payload.get("execution_launch_request_created"))
        and _clean_text(creation_payload.get("execution_launch_request_id")) == launch_request_id
    )
    observability_found = (
        source_observability_status == "observed_created"
        and bool(observability_payload.get("execution_launch_request_was_created"))
        and _clean_text(observability_payload.get("execution_launch_request_id")) == launch_request_id
    )
    created_evidence_present = (
        (
            source_status == "created"
            and bool(creation_payload.get("execution_launch_request_created"))
        )
    ...
    if not launch_request_id:
        readback_status = "blocked_missing_execution_launch_request_id"
        missing_requirements.append("execution_launch_request_id")
        blocked_actions.append("execution_launch_request_id_missing")
        next_safe_step = "provide_execution_launch_request_id"
    elif not source_available:
        readback_status = "blocked_missing_source"
        missing_requirements.append("launch_request_source_evidence")
        blocked_actions.append("application_execution_launch_request_source_missing")
        next_safe_step = "provide_guarded_launch_request_payload_or_audit_payload"
    elif creation_found or observability_found:
        readback_status = "found"
        next_safe_step = "review_execution_launch_request_details_before_any_future_execution_action"
    elif source_status.startswith("blocked") or source_observability_status in {
        "observed_blocked",
        "observed_missing_source",
        "observed_invalid_source",
    } or creation_payload.get("execution_launch_request_created") is False or created_evidence_present:
        readback_status = "not_found"
        blocked_actions.append(
            "execution_launch_request_id_mismatch"
            if created_evidence_present
            else "execution_launch_request_not_created"
        )
        next_safe_step = "resolve_guarded_execution_launch_request_blockers"
    else:
        readback_status = "insufficient_information"
        missing_requirements.append("created_launch_request_evidence")
        blocked_actions.append("application_execution_launch_request_evidence_inconclusive")
        next_safe_step = "provide_created_launch_request_evidence"

    launch_request_found = readback_status == "found"
    execution_launch_request_status = "created" if launch_request_found else "not_found"
    readback_summary = {
        "execution_launch_request_id": launch_request_id,
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "execution_launch_request_found": launch_request_found,
        "source_application_execution_launch_request_status": source_status or "missing",
        "source_application_execution_launch_request_observability_status": (
            source_observability_status or "missing"
        ),
        "read_only_source_evidence_only": True,
    }
    detail_sections = [
        {
            "section": "identifiers",
        ...
        },
        {
            ...
            "application_execution_launch_request_status": source_status or "missing",
            "application_execution_launch_request_observability_status": (
                source_observability_status or "missing"
            ),
            "execution_launch_request_created": launch_request_found,
        },
        {
            "section": "safety",
            "read_only": True,
    ...
    return {
        "application_execution_launch_request_readback_status": readback_status,
        "execution_launch_request_id": launch_request_id,
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "execution_launch_request_found": launch_request_found,
        "execution_launch_request_status": execution_launch_request_status,
        "source_application_execution_launch_request_status": source_status or "missing",
        "source_application_execution_launch_request_observability_status": (
            source_observability_status or "missing"
        ),
        ...
        "context_id": clean_context_id,
        "job_id": clean_job_id,
        "safety_metadata": _application_execution_launch_request_readback_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "application_execution_launch_request_readback",
    }


def _execution_launch_request_status_transition_preview_safety_metadata() -> Dict[str, Any]:
    return {
        "dry_run_only": True,
        "execution_launch_request_status_transition_preview_only": True,
        "read_only": True,
        "manual_only": True,
        "did_create_execution_launch_request": False,
...
def build_execution_launch_request_status_transition_preview_payload(
    *,
    execution_launch_request_id: Any = "",
    requested_transition: Any = "",
    ...
    """Preview a future execution launch request status transition without changing status."""
    ...
    current_status = _clean_text(readback_payload.get("execution_launch_request_status"))
    readback_status = _clean_text(
        readback_payload.get("application_execution_launch_request_readback_status")
    )
    readback_found = bool(readback_payload.get("execution_launch_request_found"))
    missing_requirements: List[str] = []
    blocked_actions: List[str] = []
    proposed_status = allowed_transitions.get(transition, "")
    transition_allowed = False

    if not launch_request_id:
        preview_status = "blocked_missing_execution_launch_request_id"
        transition_reason = "Execution launch request id is required before previewing a status transition."
        missing_requirements.append("execution_launch_request_id")
        blocked_actions.append("execution_launch_request_id_missing")
        next_safe_step = "provide_execution_launch_request_id"
    elif not transition:
        preview_status = "blocked_missing_requested_transition"
        transition_reason = "A requested transition is required for the dry-run preview."
        missing_requirements.append("requested_transition")
        blocked_actions.append("requested_transition_missing")
        next_safe_step = "select_execution_launch_request_transition"
    elif transition not in allowed_transitions:
        preview_status = "blocked_invalid_requested_transition"
        transition_reason = "Requested transition is not one of the supported manual preview options."
        blocked_actions.append("requested_transition_invalid")
        next_safe_step = "select_supported_execution_launch_request_transition"
    elif not readback_payload or readback_status not in {"found", "not_found"}:
        preview_status = "blocked_missing_readback"
        transition_reason = "A valid execution launch request readback payload is required."
        missing_requirements.append("application_execution_launch_request_readback_payload")
        blocked_actions.append("application_execution_launch_request_readback_missing_or_invalid")
        next_safe_step = "read_execution_launch_request_before_transition_preview"
    elif readback_status == "not_found" or not readback_found:
        preview_status = "blocked_execution_launch_request_not_found"
        transition_reason = "Readback did not find a created execution launch request/control artifact."
        blocked_actions.append("execution_launch_request_not_found")
        next_safe_step = "resolve_execution_launch_request_readback_blockers"
    elif current_status not in {"created", "pending_review"}:
        preview_status = "blocked_invalid_current_status"
        transition_reason = "Current execution launch request status is not eligible for transition preview."
        blocked_actions.append("execution_launch_request_current_status_invalid")
        next_safe_step = "review_execution_launch_request_status_before_transition"
    else:
        preview_status = "ready_for_future_status_transition"
        transition_allowed = True
        transition_reason = (
            "Execution launch request readback found a created control artifact; this dry-run "
            "previews the future status transition only and does not update anything."
        )
        next_safe_step = "require_future_guarded_status_transition_confirmation"

    return {
        "execution_launch_request_status_transition_preview_status": preview_status,
        "execution_launch_request_id": launch_request_id,
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        "requested_transition": transition,
        "current_execution_launch_request_status": current_status or "missing",
        "proposed_execution_launch_request_status": proposed_status,
        ...
        "context_id": clean_context_id,
        "job_id": clean_job_id,
        "safety_metadata": _execution_launch_request_status_transition_preview_safety_metadata(),
        "manual_surface": True,
        "read_only": True,
        "dry_run": True,
        "service_surface": "execution_launch_request_status_transition_preview",
    }
...
def _guarded_execution_launch_request_status_transition_observability_safety_metadata() -> Dict[str, Any]:
    return {
        "read_only": True,
        "observability_only": True,
        "execution_launch_request_status_transition_audit_only": True,
        "manual_only": True,
        "did_create_execution_launch_request": False,
        "did_update_execution_launch_request_status": False,
        "did_create_execution_request": False,
...
def build_guarded_execution_launch_request_status_transition_observability_payload(
    *,
    guarded_execution_launch_request_status_transition_payload: Dict[str, Any] | None = None,
    execution_launch_request_id: Any = "",
    execution_request_id: Any = "",
    approval_request_id: Any = "",
    queue_handoff_id: Any = "",
    ...
    """Summarize a guarded launch request status transition result without mutating anything."""

    source_payload = deepcopy(guarded_execution_launch_request_status_transition_payload or {})
    if not isinstance(source_payload, dict):
        source_payload = {}

    source_status = _clean_text(source_payload.get("execution_launch_request_status_transition_status"))
    launch_request_id = _clean_text(execution_launch_request_id) or _clean_text(
        source_payload.get("execution_launch_request_id")
    )
    execution_id = _clean_text(execution_request_id) or _clean_text(source_payload.get("execution_request_id"))
    ...
    handoff_id = _clean_text(queue_handoff_id) or _clean_text(source_payload.get("queue_handoff_id"))
    transition = _clean_text(source_payload.get("requested_transition"))
    previous_status = _clean_text(source_payload.get("previous_execution_launch_request_status"))
    new_status = _clean_text(source_payload.get("new_execution_launch_request_status"))
    status_updated = bool(source_payload.get("execution_launch_request_status_updated"))
    blocked_actions = [
        _clean_text(item)
        for item in list(source_payload.get("blocked_actions") or [])
        if _clean_text(item)
    ...
    if not source_payload:
        ...
        transition_was_applied = False
        transition_was_blocked = True
        blocked_actions.append("guarded_execution_launch_request_status_transition_payload_missing")
        next_safe_step = "run_guarded_execution_launch_request_status_transition_manual_action"
    elif source_status == "updated":
        observability_status = "observed_updated"
        transition_was_applied = True
        transition_was_blocked = False
        next_safe_step = "read_execution_launch_request_status_before_any_future_execution"
    elif source_status.startswith("blocked") or source_status == "insufficient_information":
        observability_status = "observed_blocked"
        transition_was_applied = False
        transition_was_blocked = True
        next_safe_step = (
            _clean_text(source_payload.get("next_safe_step"))
            or "resolve_guarded_execution_launch_request_status_transition_blockers"
        )
    elif source_status:
        observability_status = "observed_invalid_source"
        transition_was_applied = False
        transition_was_blocked = True
        blocked_actions.append("guarded_execution_launch_request_status_transition_status_unrecognized")
        next_safe_step = "rebuild_guarded_execution_launch_request_status_transition_payload"
    else:
        observability_status = "insufficient_information"
        transition_was_applied = False
        transition_was_blocked = True
        blocked_actions.append("guarded_execution_launch_request_status_transition_status_missing")
        next_safe_step = "run_guarded_execution_launch_request_status_transition_manual_action"
    ...
    audit_summary = {
        ...
        "previous_execution_launch_request_status": previous_status,
        "new_execution_launch_request_status": new_status,
        "execution_launch_request_status_updated": status_updated,
        "transition_was_applied": transition_was_applied,
        "transition_was_blocked": transition_was_blocked,
        "source_updated_execution_launch_request_status": bool(
            source_safety_payload.get("did_update_execution_launch_request_status")
        ),
        "source_executed_application": bool(source_safety_payload.get("did_execute_application")),
        "source_submitted_application": bool(source_safety_payload.get("did_submit_application")),
    }
    audit_events = [
        {
            "event_type": "guarded_execution_launch_request_status_transition_observed",
            "execution_launch_request_id": launch_request_id,
            "source_transition_status": source_status or "missing",
            "transition_was_applied": transition_was_applied,
        }
    ...
    safety_findings = {
        "source_did_update_execution_launch_request_status": bool(
            source_safety_payload.get("did_update_execution_launch_request_status")
        ),
        "observability_updated_execution_launch_request_status": False,
        "observability_created_execution_launch_request": False,
        "observability_created_execution_request": False,
        "observability_updated_execution_request_status": False,
        "observability_mutated_queue": False,
    ...
    return {
        "execution_launch_request_status_transition_observability_status": observability_status,
        "source_execution_launch_request_status_transition_status": source_status or "missing",
        "execution_launch_request_id": launch_request_id,
        "execution_request_id": execution_id,
        "approval_request_id": request_id,
        "queue_handoff_id": handoff_id,
        ...
        "new_execution_launch_request_status": new_status,
        "execution_launch_request_status_updated": status_updated,
        "execution_launch_request_status_transition_was_blocked": transition_was_blocked,
        "execution_launch_request_status_transition_was_applied": transition_was_applied,
        "audit_summary": audit_summary,
        "audit_events": audit_events,
        "safety_findings": safety_findings,
        "blocked_actions": list(dict.fromkeys(blocked_actions)),
        ...
        "job_id": _clean_text(job_id) or _clean_text(source_payload.get("job_id")),
        "safety_metadata": (
            _guarded_execution_launch_request_status_transition_observability_safety_metadata()
        ),
        "manual_surface": True,
        "read_only": True,
        "service_surface": "guarded_execution_launch_request_status_transition_observability",
    }
...
def _agentic_workflow_dry_run_from_artifacts(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    result_json = _artifact_json_by_name(rows, "agentic_workflow_dry_run_result.json")
    report_markdown = _artifact_text_by_name(rows, "agentic_workflow_dry_run_report.md")
    return {
