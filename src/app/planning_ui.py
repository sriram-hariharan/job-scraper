...
def _safe_json_script(payload: dict) -> str:
    """Serialize payload for a literal <script> assignment without allowing
...
def advanced_diagnostics(
    ...
    artifact_context_exists = bool(
        (tailoring_json or "").strip()
        or (tailoring_md or "").strip()
        or (tailoring_llm_json or "").strip()
        or (packet_json or "").strip()
    )
    has_context = artifact_context_exists or bool(selected_scan_context)
    invalid_context = bool(selected_scan_id) and not selected_scan_context and not artifact_context_exists

    if has_context:
        mode = "context"
    elif invalid_context:
        mode = "invalid"
    elif not scan_context_options:
        mode = "empty"
    else:
        mode = "hub"
    ...
    if mode == "context":
        ...
        context_id_source = (
            ...
            or tailoring_json
            or tailoring_md
            or tailoring_llm_json
        )
        context_payload = {
            "company": company or "-",
            "title": title or "-",
            "resume": Path(raw_resume_name).stem.replace("_", " ") if raw_resume_name else "-",
            "status": status or "Scan diagnostics context loaded",
            "contextId": (
                Path(str(context_id_source).replace("\\", "/")).name
                if context_id_source
                else "Scan context"
            ),
            "backToScanHref": f"/scan-workspace?{back_query}",
        }
    ...
    initial_state = {
        ...
        "savedScanOptions": [
            {
                "scanId": str(option.get("scan_id") or ""),
                "company": str(option.get("company") or ""),
                "title": str(option.get("title") or ""),
                "resume": str(option.get("resume") or ""),
                "status": str(option.get("status") or ""),
                "primary": str(option.get("primary") or ""),
                "secondary": str(option.get("secondary") or ""),
            }
            for option in scan_context_options
        ],
        "selectedScanId": selected_scan_id,
