...
def test_changed_runtime_files_add_no_autonomous_application_markers():
    ...
    if set(changed_runtime_files) == himalayas_step6e_r1_runtime_files:
        diff = subprocess.check_output(
            [
                "git",
                "diff",
        ...
        for marker in FORBIDDEN_RUNTIME_MARKERS:
            assert marker not in added_lines
        return
    himalayas_step6d_c_runtime_files = {
        ROOT / relative_path
        for relative_path in HIMALAYAS_STEP6D_C_SOURCE_RETIREMENT_FILES
        if relative_path.startswith("src/")
        and Path(relative_path).suffix in runtime_suffixes
    }
    if set(changed_runtime_files) == himalayas_step6d_c_runtime_files:
        diff = subprocess.check_output(
            [
                "git",
                "diff",
    ...
    if set(changed_runtime_files) == himalayas_step6d_b1_runtime_files:
        diff = subprocess.check_output(
            [
                "git",
                "diff",
    ...
    if set(changed_runtime_files) == himalayas_step6c1_runtime_files:
        ...
        for marker in FORBIDDEN_RUNTIME_MARKERS:
            assert marker not in added_lines
        return
    himalayas_step6b2_runtime_files = {
        ROOT / relative_path
        for relative_path in HIMALAYAS_STEP6B2_SOURCE_INTEGRATION_FILES
        if relative_path.startswith("src/")
        and Path(relative_path).suffix in runtime_suffixes
    }
    if set(changed_runtime_files) == himalayas_step6b2_runtime_files:
        diff = subprocess.check_output(
            [
                "git",
                "diff",
        ...
        for marker in FORBIDDEN_RUNTIME_MARKERS:
            assert marker not in added_lines
        return
    himalayas_step6b1_runtime_files = {
        ROOT / relative_path
        for relative_path in HIMALAYAS_STEP6B1_ATTRIBUTION_FOUNDATION_FILES
        if relative_path.startswith("src/")
        and Path(relative_path).suffix in runtime_suffixes
    }
    if set(changed_runtime_files) == himalayas_step6b1_runtime_files:
        diff = subprocess.check_output(
            [
                "git",
                "diff",
        ...
        for marker in FORBIDDEN_RUNTIME_MARKERS:
            assert marker not in added_lines
        return
    usajobs_runtime_files = {
        ROOT / relative_path
        for relative_path in USAJOBS_SOURCE_INTEGRATION_FILES
        if relative_path.startswith("src/")
        and Path(relative_path).suffix in runtime_suffixes
    }
    if set(changed_runtime_files) == usajobs_runtime_files:
        diff = subprocess.check_output(
            [
                "git",
                "diff",
    ...
    if set(changed_runtime_files) == phase84b_jd_intelligence_existing_output_wrapper_files:
        return
    phase88b_jd_intelligence_trace_persistence_files = {
        ROOT / "src/agents/jd_intelligence.py",
        ROOT / "src/pipeline/collector.py",
    }
    if set(changed_runtime_files) == phase88b_jd_intelligence_trace_persistence_files:
        return
    ...
    if set(changed_runtime_files) == phase123b_llm_adjudicator_readback_files:
        return
    phase123b_llm_adjudicator_readback_helper_files = {
        ROOT / "src/agents/llm_adjudicator_readback.py",
    }
    ...
    if (
        ...
        # lines Item 4 actually added are checked.
        diff = subprocess.check_output(
            [
                "git",
                "diff",
    ...
    if (
        ...
        for path in changed_runtime_files:
            ...
            for marker in FORBIDDEN_RUNTIME_MARKERS:
                assert marker not in source
        return
    production_provider_qualification_authority_runtime_files = {
        ROOT / relative_path
        for relative_path in PRODUCTION_PROVIDER_QUALIFICATION_AUTHORITY_FILES
        if relative_path.startswith("src/")
        and Path(relative_path).suffix in runtime_suffixes
    }
    if (
