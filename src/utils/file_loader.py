...
def _postgres_discovery_lines(path: str):
    ...
    if (
        safe_path in {"discovery://company_domains", "postgres://discovery/company_domains"}
        or safe_path == "data/company_domains.txt"
        or name == "company_domains.txt"
    ):
    ...
    ats = ""
    if safe_path.startswith("discovery://ats/"):
        ats = safe_path.rsplit("/", 1)[-1].strip().lower()
    elif safe_path.startswith("postgres://discovery/ats/"):
        ats = safe_path.rsplit("/", 1)[-1].strip().lower()
    else:
