...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Apply operator-decisions storage SQL artifact to Postgres through psql."
    )
    parser.add_argument(
        "--database-url",
        default="",
...
def main() -> int:
    ...
    if not contract_health.get("all_checks_pass", False) and not args.allow_contract_drift:
        raise SystemExit(
            "Operator-decisions contract health check failed. Refusing to apply SQL while artifacts are drifting. "
            "Re-run after fixing the schema artifact mismatch, or pass --allow-contract-drift if you intentionally want to override."
        )

    sql_payload: Dict[str, str] = operator_decisions_schema_sql_payload()
