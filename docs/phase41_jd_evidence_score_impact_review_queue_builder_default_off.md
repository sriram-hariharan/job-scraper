...
Phase 41A adds the JD evidence score impact review queue builder. This is a capability step on the revised path. It is not another safety-wrapper chain.

The helper performs deterministic review queue building. It builds prioritized operator review queues from score impact review packets, prioritizes red flag review and blocked score preview rows, and preserves deterministic order for equal priorities. It produces manual-review queue only output and preserves existing score fields.
