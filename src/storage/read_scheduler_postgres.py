...
def _build_scheduler_status_sql(limit: int) -> str:
    return f"""
WITH latest_rows AS (
...
SELECT json_build_object(
    'job_definition_count', (SELECT COUNT(*) FROM scheduler_job_definitions),
    'active_job_count', (SELECT COUNT(*) FROM scheduler_job_definitions WHERE is_active),
    'run_history_count', (SELECT COUNT(*) FROM scheduler_run_history),
    'success_count', (SELECT COUNT(*) FROM scheduler_run_history WHERE status = 'succeeded'),
    'failure_count', (SELECT COUNT(*) FROM scheduler_run_history WHERE status = 'failed'),
    'latest_runs_by_job',
        COALESCE(
    ...
    'recent_runs',
        COALESCE(
            (SELECT json_agg(row_to_json(recent_rows) ORDER BY recent_rows.started_at DESC) FROM recent_rows),
            '[]'::json
        )
);
""".strip()
...
def _run_psql_json_query(
    *,
    ...
    if not stdout:
        raise SystemExit("psql returned empty output for scheduler Postgres status query.")

    try:
        data = json.loads(stdout)
    except json.JSONDecodeError as exc:
        raise SystemExit(
            f"Failed to parse scheduler Postgres status JSON from psql output: {exc}"
        ) from exc

    if not isinstance(data, dict):
        raise SystemExit("Scheduler Postgres status query did not return a JSON object.")

    payload["data"] = data
    return payload


def get_scheduler_postgres_status_payload(
    *,
    limit: int = 10,
    database_url: str = "",
    database_url_env: str = "DATABASE_URL",
    ...
    sql = _build_scheduler_status_sql(normalized_limit)
