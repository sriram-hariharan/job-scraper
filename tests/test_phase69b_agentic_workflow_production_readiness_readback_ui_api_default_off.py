...
def _client(monkeypatch) -> TestClient:
    ...
    monkeypatch.setattr(api, "_auth_owner_user_id", lambda request: "owner-69b")
    monkeypatch.setattr(api, "_auth_owner_email", lambda request: "owner69b@example.test")
...
def test_default_api_readback_does_not_perform_manual_mutation(monkeypatch):
    ...
    assert checkpoint["readback_phase"] == "69B"
    assert checkpoint["api_readback"] is True
    assert checkpoint["ui_readback"] is True
...
def test_production_readiness_api_readback_exposes_completed_workflow(monkeypatch):
    payload = _post_scan(_client(monkeypatch))
    checkpoint = payload["agentic_workflow_production_readiness_checkpoint"]
    ...
    assert checkpoint["backend_agentic_workflow_complete"] is True
    assert checkpoint["backend_agentic_workflow_completion_status"] == "complete"


def test_core_llm_inference_is_workflow_automatic(monkeypatch):
    monkeypatch.setattr(
        services,
...
def test_analysis_and_scoring_responsibilities_are_not_mixed(monkeypatch):
...
def test_planning_actions_are_readback_only_without_artifact_creation(monkeypatch):
...
def test_human_handoff_path_does_not_imply_ats_automation(monkeypatch):
    _patch_storage(monkeypatch, stored_payload=_stored_scan_payload())
    ...
    assert checkpoint["human_only_application_boundary"] is True
...
def test_no_live_provider_call_is_made_by_phase69b_readback(monkeypatch):
    calls = []
    _patch_storage(monkeypatch, stored_payload=_stored_scan_payload())
    monkeypatch.setattr(
        services,
...
def test_phase56_59_60_67_68_and_69a_readbacks_remain_intact(monkeypatch):
    _patch_storage(monkeypatch, stored_payload=_stored_scan_payload())
...
def test_docs_include_phase69b_markers():
    ...
    for marker in (
        "agentic workflow production readiness readback",
