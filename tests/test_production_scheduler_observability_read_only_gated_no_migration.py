...
def _approved_production_wiring_gate():
    return queue.production_scheduler_wiring_decision_payload(
        approval_request_id="approval_1",
...
def _assert_read_only_no_side_effects(result):
    assert result["production_scheduler_observability_read_only"] is True
...
def test_observability_blocks_without_recorded_approval():
    result = queue.production_scheduler_observability_decision_payload(
        approval_request_id="approval_1",
        production_scheduler_wiring_output=_approved_production_wiring_gate(),
    )

    assert result["production_scheduler_observability_gate_enabled"] is True
    assert result["production_scheduler_observability_decision_enabled"] is True
    assert result["production_scheduler_observability_allowed"] is False
    assert result["production_scheduler_observability_status"] == "blocked"
    assert "approval_record_provider_unavailable" in result[
        "production_scheduler_observability_reason_codes"
    ]
    _assert_read_only_no_side_effects(result)


def test_observability_blocks_missing_record_from_fake_provider():
    result = queue.production_scheduler_observability_decision_payload(
        approval_request_id="approval_1",
        production_scheduler_wiring_output=_approved_production_wiring_gate(),
...
def test_observability_blocks_non_approved_supported_status():
    result = queue.production_scheduler_observability_decision_payload(
        approval_request_id="approval_1",
        production_scheduler_wiring_output=_approved_production_wiring_gate(),
...
def test_observability_blocks_unsupported_approval_status():
    result = queue.production_scheduler_observability_decision_payload(
        approval_request_id="approval_1",
        production_scheduler_wiring_output=_approved_production_wiring_gate(),
...
def test_observability_blocks_without_approval_gated_execution():
    wiring_gate = deepcopy(_approved_production_wiring_gate())
    wiring_gate["approval_gated_execution_allowed"] = False
    wiring_gate["approval_gated_execution_status"] = "blocked"
...
def test_observability_blocks_without_gated_application_submission():
    wiring_gate = deepcopy(_approved_production_wiring_gate())
    wiring_gate["application_submission_allowed"] = False
    wiring_gate["application_submission_status"] = "blocked"
...
def test_observability_blocks_without_scheduler_background_decision():
    wiring_gate = deepcopy(_approved_production_wiring_gate())
    wiring_gate["scheduler_background_execution_allowed"] = False
    wiring_gate["scheduler_background_execution_status"] = "blocked"
...
def test_observability_blocks_without_live_scheduler_decision():
    wiring_gate = deepcopy(_approved_production_wiring_gate())
    wiring_gate["live_scheduler_execution_allowed"] = False
    wiring_gate["live_scheduler_execution_status"] = "blocked"
...
def test_observability_blocks_without_production_scheduler_wiring_decision():
    wiring_gate = deepcopy(_approved_production_wiring_gate())
    wiring_gate["production_scheduler_wiring_allowed"] = False
    wiring_gate["production_scheduler_wiring_status"] = "blocked"
    ...
    assert result["production_scheduler_observability_allowed"] is False
    assert "production_scheduler_wiring_not_allowed" in result[
        "production_scheduler_observability_reason_codes"
    ]
    assert "production_scheduler_wiring_status_not_passed" in result[
        "production_scheduler_observability_reason_codes"
    ]
    _assert_read_only_no_side_effects(result)


def test_observability_allowed_read_only_only_after_all_gates_pass():
    result = queue.production_scheduler_observability_decision_payload(
        approval_request_id="approval_1",
        production_scheduler_wiring_output=_approved_production_wiring_gate(),
        approval_record_provider=_provider("approved"),
    ...
    assert result["production_scheduler_observability_status"] == "passed"
    assert result["production_scheduler_observability_reason_codes"] == []
    _assert_read_only_no_side_effects(result)


def test_observability_preserves_all_gate_blocks():
    wiring_gate = deepcopy(_approved_production_wiring_gate())
    wiring_gate["blocked_by_queue_safety_gate"] = True
    wiring_gate["queue_safety_gate_passed"] = False
    wiring_gate["queue_safety_gate_status"] = "failed"
    wiring_gate["approval_gated_execution_allowed"] = False
    wiring_gate["approval_gated_execution_status"] = "blocked"
    wiring_gate["application_submission_allowed"] = False
    wiring_gate["application_submission_status"] = "blocked"
...
def test_observability_uses_fakeable_storage_boundary():
    calls = []
...
def test_observability_queue_boundary_has_no_emitters_workers_or_migration_constructs():
