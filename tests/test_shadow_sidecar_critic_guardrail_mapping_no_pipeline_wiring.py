...
def _critic_sidecar_input(**config):
    return shadow_sidecar.build_shadow_sidecar_input_payload(
        run_id="run_critic_shadow",
        batch_id="batch_critic_shadow",
        job_id="job_critic_shadow",
        stage_name="critic_guardrail_shadow_sidecar",
        source_deterministic_stage="application_priority",
        source_deterministic_status="completed",
        source_deterministic_score=0.84,
        source_deterministic_decision="human_review_required",
        source_deterministic_reason_codes=["tailoring_requires_guardrail_review"],
        job_payload={
            "title": "Data Platform Engineer",
            "critic_evidence_refs": [
                "existing_trace_context.approved_suggestions",
            ],
        ...
        existing_trace_context={
            ...
            "critic_reason_codes": ["existing_trace_critic_context"],
            "unsupported_claim_risks": ["metric_requires_evidence"],
            "evidence_gaps": ["ownership_scope_needs_review"],
        },
        sidecar_config=config,
        agent_name="critic_guardrail",
        started_at_utc="2026-06-16T13:00:00Z",
        completed_at_utc="2026-06-16T13:00:01Z",
        duration_ms=1000,
    )
...
def test_critic_mapping_is_not_enabled_when_global_sidecar_flag_is_disabled():
    payload = shadow_sidecar.run_shadow_sidecar_agent(
        sidecar_input=_critic_sidecar_input(
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_CRITIC_GUARDRAIL_ENABLED=True,
        )
    )

    assert payload["sidecar_stage_status"] == "not_enabled"
    assert payload["agent_name"] == "critic_guardrail"
    assert payload["source_deterministic_decision"] == "human_review_required"
    assert "global_sidecar_flag_disabled" in payload["agent_reason_codes"]
    _assert_no_mutation_safety(payload)


def test_critic_mapping_is_skipped_when_per_agent_flag_is_disabled():
    payload = shadow_sidecar.run_shadow_sidecar_agent(
        sidecar_input=_critic_sidecar_input(
    ...
    assert payload["source_deterministic_score"] == 0.84
    _assert_no_mutation_safety(payload)


def test_kill_switch_blocks_critic_sidecar_work():
    payload = shadow_sidecar.run_shadow_sidecar_agent(
        sidecar_input=_critic_sidecar_input(
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_SIDECAR_ENABLED=True,
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_CRITIC_GUARDRAIL_ENABLED=True,
...
def test_provider_calls_are_not_made_for_critic_mapping_in_tests():
    calls = []
...
def test_critic_mapping_returns_deterministic_fallback_with_guardrail_hints():
    payload = shadow_sidecar.run_shadow_sidecar_agent(
        sidecar_input=_critic_sidecar_input(
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_SIDECAR_ENABLED=True,
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_CRITIC_GUARDRAIL_ENABLED=True,
    ...
    assert "patch_supported_by_resume_evidence" in payload["agent_reason_codes"]
    assert "resume_evidence_available" in payload["agent_reason_codes"]
    assert "existing_trace_critic_context" in payload["agent_reason_codes"]
    assert "critic_shadow_signals_observed" in payload["agent_reason_codes"]
    assert "existing_trace_context.approved_suggestions" in payload["agent_evidence_refs"]
    assert "existing_trace_context.critic_summary" in payload["agent_evidence_refs"]
    assert "human_review_required" in payload["agent_risk_flags"]
    assert "metric_requires_evidence" in payload["agent_risk_flags"]
    assert "no_real_workflow_block_applied" in payload["agent_blocking_findings"]
    assert "ownership_scope_needs_review" in payload["agent_blocking_findings"]
    _assert_no_mutation_safety(payload)


def test_critic_mapping_preserves_source_decision_and_trace_sections():
    payload = shadow_sidecar.run_shadow_sidecar_agent(
        sidecar_input=_critic_sidecar_input(
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_SIDECAR_ENABLED=True,
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_CRITIC_GUARDRAIL_ENABLED=True,
    ...
    assert payload["source_deterministic_reason_codes"] == [
        "tailoring_requires_guardrail_review"
    ]
...
def test_critic_mapping_does_not_block_or_mutate_real_workflow():
    payload = shadow_sidecar.run_shadow_sidecar_agent(
        sidecar_input=_critic_sidecar_input(
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_SIDECAR_ENABLED=True,
            APPLYLENS_AGENTIC_PIPELINE_SHADOW_CRITIC_GUARDRAIL_ENABLED=True,
    ...
    assert payload["agent_blocking_findings"]
...
def test_shadow_sidecar_has_no_pipeline_api_ui_service_or_critic_provider_wiring():
    ...
    forbidden = [
        ...
        "src.app.api",
        "agentic_review.js",
        "build_critic_guardrail_dry_run_payload",
    ...
    assert "APPLYLENS_AGENTIC_PIPELINE_SHADOW_CRITIC_GUARDRAIL_ENABLED" in source
