...
def test_first_synthetic_fixture_payload_implementation_covers_phase_73a_contract():
    ...
    assert not {
        ...
        if (
            path.endswith("fixture_validator.py")
            or path.endswith("agentic_fixture_validator.py")
            or path.endswith("validate_fixture.py")
            or path.endswith("validator_cli.py")
        )
    }
    assert not {
        path
        for path in repo_files
        if path.startswith("tests/") and "fixture_validator" in path
    }
