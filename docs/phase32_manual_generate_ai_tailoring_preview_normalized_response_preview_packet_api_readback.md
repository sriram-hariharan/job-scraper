...
Phase 32B adds API readback only for the Phase 32A manual Generate AI Tailoring preview normalized response preview packet contract.

The route is `GET /api/manual-generate-ai-tailoring-preview-normalized-response-preview-packet-contract`. It returns deterministic placeholder readback metadata from `build_manual_generate_ai_tailoring_preview_normalized_response_preview_packet_contract`. It does not accept user edits, does not persist user edits, does not call providers, does not call network, and does not dispatch.

## Readback boundary

This API readback is normalized response preview packet contract only, preview packet only, default-off, read-only, advisory-only, and manual-review only. User trigger required. Operator confirmation required. Manual acceptance required. Provider response normalization required. Normalized provider response required. Provider response validation required. Preview packet policy required. Provider configuration required.
...
## Exact Phase 32B safety markers
