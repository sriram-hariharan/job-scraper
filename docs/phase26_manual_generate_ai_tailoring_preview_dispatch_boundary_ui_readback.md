...
Phase 26C adds UI readback only for the Phase 26B manual Generate AI Tailoring preview dispatch-boundary API readback.

The UI passively displays the read-only payload from `/api/manual-generate-ai-tailoring-preview-dispatch-boundary-contract` with `renderManualGenerateAiTailoringPreviewDispatchBoundaryReadbackSection`. It does not add a real Generate AI Tailoring button or control, does not add a dispatch control, and does not create any action path.

## Readback boundary

This UI readback is dispatch-boundary contract only, default-off, read-only, advisory-only, and manual-review only. User trigger required. Operator confirmation required. Manual acceptance required before any future resume edit.

This UI does not dispatch. It does not call network except the optional explicitly gated read-only GET fetch. It does not generate AI tailoring. It does not call tailoring runtime. It does not call providers. It does not create real tailoring output. It does not create resume rewrites. It does not overwrite resumes. It does not mutate resumes. It does not persist data. It does not write to database. It does not execute applications. It does not submit applications.
...
## Exact Phase 26C safety markers
