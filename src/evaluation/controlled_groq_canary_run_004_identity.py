"""Offline identity and inactive authorization template for Groq run 004."""
...
from src.evaluation.controlled_groq_canary_run_004_plan import (
    RUN_004_IDENTIFIER as PLAN_RUN_004_IDENTIFIER,
...
RUN_004_IDENTITY_VERSION = "controlled-groq-canary-run-004-identity-v1"
RUN_004_AUTHORIZATION_TEMPLATE_VERSION = (
    "controlled-groq-canary-run-004-authorization-template-v1"
)
...
RUN_004_ARTIFACT_PATHS = {
    "pricing": (
        "outputs/provider_benchmark/phase11_groq_canary_pricing_004.json"
    ),
    "authorization": (
        "outputs/provider_benchmark/"
        "phase11_groq_canary_authorization_004.json"
    ),
    "checkpoint": (
        "outputs/provider_benchmark/"
        "phase11_groq_canary_checkpoint_004.json"
    ),
    "result": (
        "outputs/provider_benchmark/phase11_groq_canary_result_004.json"
    ),
}
...
def _validated_plan() -> Dict[str, Any]:
    ...
    _require(
        PLAN_RUN_004_IDENTIFIER == RUN_004_IDENTIFIER
        and plan["run_identifier"] == RUN_004_IDENTIFIER,
        "run-004 plan identifier changed",
    )
    _require(
...
def _expected_identity_contract() -> Dict[str, Any]:
    ...
    return {
        ...
        "contract_kind": (
            "offline-run-004-two-case-groq-120b-quality-identity"
        ),
        ...
        "future_artifact_identities": deepcopy(RUN_004_ARTIFACT_PATHS),
        "protected_prior_artifacts": {
            "run_001": {
                ...
                "checkpoint_as_run_004_initial_state_allowed": False,
                "result_as_run_004_initial_state_allowed": False,
            },
            "run_002": {
                "artifact_paths": deepcopy(
                    PROTECTED_RUN_002_ARTIFACT_PATHS
...
def build_run_004_identity_contract() -> Dict[str, Any]:
    contract = _expected_identity_contract()
    validate_run_004_identity_contract(contract)
    return deepcopy(contract)


def validate_run_004_identity_contract(contract: Dict[str, Any]) -> bool:
    _require(
        isinstance(contract, dict) and set(contract) == _IDENTITY_FIELDS,
        "run-004 identity fields must match the exact schema",
    )
    ...
    _require(
        ...
        "run-004 identity schedule must contain exactly two plan rows",
    )
    _require(
        not _contains_forbidden_key(contract),
        "run-004 identity contains a forbidden field",
    )
    _require(
        contract == _expected_identity_contract(),
        "run-004 identity differs from the committed plan identity",
    )
    return True


def serialize_run_004_identity_contract(
    ...
    validate_run_004_identity_contract(payload)
    return _canonical_json(payload)


def run_004_identity_sha256(
    contract: Dict[str, Any] | None = None,
) -> str:
    return sha256(
        serialize_run_004_identity_contract(contract).encode("utf-8")
    ).hexdigest()
...
def _expected_authorization_template() -> Dict[str, Any]:
    ...
    return {
        ...
        "run_004_identity_version": RUN_004_IDENTITY_VERSION,
        "run_identifier": RUN_004_IDENTIFIER,
        "run_004_identity_sha256": run_004_identity_sha256(identity),
        "run_004_plan_version": RUN_004_PLAN_VERSION,
        "run_004_plan_sha256": _PINNED_PLAN_SHA256,
        "candidate_provider_models": [
        ...
        "reserved_artifact_paths": deepcopy(RUN_004_ARTIFACT_PATHS),
        "maximum_observed_cost_per_model": {
...
def build_run_004_authorization_template() -> Dict[str, Any]:
    template = _expected_authorization_template()
    validate_run_004_authorization_template(template)
    return deepcopy(template)


def validate_run_004_authorization_template(
    template: Dict[str, Any],
) -> bool:
    _require(
        isinstance(template, dict)
        and set(template) == _AUTHORIZATION_TEMPLATE_FIELDS,
        "run-004 authorization template fields must match the exact schema",
    )
    _require(
        not _contains_forbidden_key(template),
        "run-004 authorization template contains a forbidden field",
    )
    _require(
        template == _expected_authorization_template(),
        "run-004 authorization template changed or gained authority",
    )
    return True


def serialize_run_004_authorization_template(
    ...
    validate_run_004_authorization_template(payload)
    return _canonical_json(payload)


def run_004_authorization_template_sha256(
    template: Dict[str, Any] | None = None,
) -> str:
    return sha256(
        serialize_run_004_authorization_template(template).encode("utf-8")
    ).hexdigest()
