...
function shouldRenderTailoringAgentOpportunityFixture(search = null) {
  const query = search === null
    ? (typeof window !== "undefined" ? window.location.search : "")
    : String(search || "");
  return new URLSearchParams(query).get(
    "tailoring_agent_opportunity_fixture",
  ) === "1";
}

function buildTailoringAgentOpportunityFixtureResult() {
  return {
    local_fixture_preview: true,
    contract_version: "phase-23a-tailoring-agent-opportunity-contract-v1",
    contract_status: "tailoring_agent_opportunity_contract_ready_for_manual_review",
    tailoring_agent_opportunity_contract_enabled: true,
    default_off: true,
    read_only: true,
    advisory_only: true,
    manual_review_only: true,
    ...
    tailoring_opportunity_summary: "Identified 3 caller-supplied tailoring opportunities for manual review.",
    tailoring_opportunities: [
      {
        ...
        suggested_next_step: "review_existing_resume_evidence_for_supported_alignment",
      },
      {
        ...
        manual_review_required: true,
        generate_ai_tailoring_allowed_now: false,
        suggested_next_step: "review_missing_evidence_before_tailoring",
      },
      {
        ...
        manual_review_required: true,
        generate_ai_tailoring_allowed_now: false,
        suggested_next_step: "review_tailoring_opportunity_under_manual_control",
      },
    ],
    future_user_triggered_action: "Generate AI Tailoring",
    next_safe_step: "review_tailoring_opportunities_without_generating_ai_tailoring",
  };
}

function withTailoringAgentOpportunityFixture(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
    ...
    : {};
  if (
    hasAgentTraceSummaryObject(source.tailoring_agent_opportunity_result)
  ...
  return {
    ...source,
    tailoring_agent_opportunity_result: (
      buildTailoringAgentOpportunityFixtureResult()
    ),
  };
}
...
function renderTailoringAgentOpportunityReadbackSection(
  ...
  const result = hasAgentTraceSummaryObject(
    tracePayload?.tailoring_agent_opportunity_result,
  )
    ? tracePayload.tailoring_agent_opportunity_result
    : {};
  if (!Object.keys(result).length) return "";

  const opportunities = Array.isArray(result.tailoring_opportunities)
    ? result.tailoring_opportunities
    : [];
  const inputs = hasAgentTraceSummaryObject(result.opportunity_inputs)
    ? result.opportunity_inputs
    : {};
  const packet = hasAgentTraceSummaryObject(
    inputs.core_agent_evidence_packet,
  )
    ? inputs.core_agent_evidence_packet
    : {};
  const manualReviewPacket = hasAgentTraceSummaryObject(
    packet.manual_review_evidence_packet,
  )
    ? packet.manual_review_evidence_packet
    : {};
  const missingEvidence = Array.isArray(
    manualReviewPacket.missing_evidence_fields,
  )
    ? manualReviewPacket.missing_evidence_fields
    : [];
  const previewBadge = result.local_fixture_preview === true
  ...
  const opportunityRows = opportunities.length
    ? opportunities.map((opportunity) => `
        <article class="tailoring-agent-opportunity-readback__item">
          <div class="agent-trace-counts">
            ${renderWorkflowSummaryMetric("Opportunity type", opportunity?.opportunity_type || "unknown")}
            ${renderWorkflowSummaryMetric("Source", opportunity?.source || "unknown")}
            ${renderWorkflowSummaryMetric("Signal", Array.isArray(opportunity?.signal) ? opportunity.signal.map(String).join(", ") : String(opportunity?.signal || "not supplied"))}
            ${renderWorkflowSummaryMetric("Manual review required", opportunity?.manual_review_required === true ? "yes" : "no")}
            ${renderWorkflowSummaryMetric("Generate AI tailoring allowed now", opportunity?.generate_ai_tailoring_allowed_now === true ? "yes" : "no")}
            ${renderWorkflowSummaryMetric("Suggested next step", opportunity?.suggested_next_step || "none")}
          </div>
        </article>
      `).join("")
    : `<p class="agentic-review-muted">No supplied tailoring opportunities are available for passive review.</p>`;
  return `
    <article class="agent-trace-summary tailoring-agent-opportunity-readback" aria-label="Tailoring-agent opportunity readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Tailoring-Agent Opportunity Readback</h4>
          <p>Read-only, advisory-only, manual-review only. This UI only identifies tailoring opportunities and does not generate AI tailoring.</p>
          <p class="agentic-review-muted">The tailoring agent remains separate from final scoring. Manual user control required.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(previewBadge)}</span>
      </div>
      <div class="tailoring-agent-opportunity-readback__safety-labels" aria-label="Tailoring-agent opportunity safety labels">
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
      </div>
      <div class="agent-trace-counts tailoring-agent-opportunity-readback__metrics">
        ${renderWorkflowSummaryMetric("Contract status", result.contract_status || "unknown")}
        ${renderWorkflowSummaryMetric("Opportunity count", opportunities.length)}
        ${renderWorkflowSummaryMetric("Missing evidence summary", missingEvidence.length ? missingEvidence.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Future user-triggered action", result.future_user_triggered_action || "Generate AI Tailoring")}
        ${renderWorkflowSummaryMetric("No provider calls", result.no_provider_calls === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("No database writes", result.no_database_writes === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("No persistence", result.no_persistence === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("No mutation", result.no_mutation === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("No execution", result.no_execution === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("No submission", result.no_submission === true ? "yes" : "no")}
      </div>
      <div class="tailoring-agent-opportunity-readback__boundary">
        <strong>Tailoring safety boundary</strong>
        <span>Generate AI Tailoring is later user-triggered only; there is no control for it in this phase.</span>
        <span>Generated tailoring suggestions remain preview/manual-review only unless the user accepts edits.</span>
        <span>No silent resume rewrite. No automatic resume overwrite. No resume mutation. No application submission.</span>
        <span>No auto-apply. No auto-submit. No autonomous application execution. No automatic job application submission.</span>
      </div>
      <div class="tailoring-agent-opportunity-readback__items">
        ${opportunityRows}
      </div>
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Opportunity inputs", inputs, { helper: "Caller-supplied evidence only." })}
        ${renderAgentTraceReadOnlyDetails("Tailoring opportunity summary", result.tailoring_opportunity_summary || "", { helper: "Read-only missing-evidence and opportunity summary." })}
      </div>
    </article>
  `;
}
...
function shouldRenderGenerateAiTailoringActionBoundaryFixture(
  ...
  const query = search === null
    ...
    : String(search || "");
  return new URLSearchParams(query).get(
    "generate_ai_tailoring_action_boundary_fixture",
  ) === "1";
}

function buildGenerateAiTailoringActionBoundaryFixtureResult() {
  return {
    local_fixture_preview: true,
    contract_version: "phase-23d-generate-ai-tailoring-action-boundary-v1",
    action_boundary_status: (
      "generate_ai_tailoring_action_boundary_ready_for_future_action"
    ),
    generate_ai_tailoring_action_boundary_enabled: true,
    default_off: true,
    read_only: true,
    advisory_only: true,
    manual_review_only: true,
...
function withGenerateAiTailoringActionBoundaryFixture(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  if (
    hasAgentTraceSummaryObject(
      source.generate_ai_tailoring_action_boundary_result,
    )
  ...
  return {
    ...
    generate_ai_tailoring_action_boundary_result: (
      buildGenerateAiTailoringActionBoundaryFixtureResult()
    ),
  };
}
...
function renderGenerateAiTailoringActionBoundaryReadbackSection(
  ...
  const result = hasAgentTraceSummaryObject(
    tracePayload?.generate_ai_tailoring_action_boundary_result,
  )
    ? tracePayload.generate_ai_tailoring_action_boundary_result
    : {};
  if (!Object.keys(result).length) return "";

  const actionInputs = hasAgentTraceSummaryObject(result.action_inputs)
    ? result.action_inputs
    : {};
  const safety = hasAgentTraceSummaryObject(result.safety_metadata)
    ? result.safety_metadata
    : {};
  const previewBadge = result.local_fixture_preview === true
    ...
    : result.ui_api_fetch_failed === true
      ...
      : "Default-off readback";
  return `
    <article class="agent-trace-summary generate-ai-tailoring-action-boundary-readback" aria-label="Generate AI Tailoring action-boundary readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Generate AI Tailoring Action-Boundary Readback</h4>
          <p>Read-only, advisory-only, manual-review only. This is action-boundary readback only and does not generate AI tailoring.</p>
          <p class="agentic-review-muted">It does not call tailoring runtime, does not call providers, does not create resume rewrites, does not overwrite resumes, and does not submit app<span>lications</span>.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(previewBadge)}</span>
      </div>
      <div class="generate-ai-tailoring-action-boundary-readback__safety-labels" aria-label="Generate AI Tailoring action-boundary safety labels">
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
        <span>Action-boundary readback only</span>
      </div>
      <div class="agent-trace-counts generate-ai-tailoring-action-boundary-readback__metrics">
        ${renderWorkflowSummaryMetric("Action boundary status", result.action_boundary_status || "unknown")}
        ${renderWorkflowSummaryMetric("User trigger required", result.user_trigger_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("User triggered", result.user_triggered === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Action allowed", result.action_allowed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Action blocked reason", result.action_blocked_reason || "none")}
        ${renderWorkflowSummaryMetric("Future action name", result.future_action_name || "Generate AI Tailoring")}
        ${renderWorkflowSummaryMetric("AI tailoring generation performed", result.ai_tailoring_generation_performed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Tailoring provider call performed", result.tailoring_provider_call_performed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Tailoring runtime call performed", result.tailoring_runtime_call_performed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Resume rewrite performed", result.resume_rewrite_performed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Resume overwrite performed", result.resume_overwrite_performed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Application submission performed", result.application_submission_performed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Preview only", result.preview_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual acceptance required", result.manual_acceptance_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Next safe step", result.next_safe_step || "none")}
      </div>
      <div class="generate-ai-tailoring-action-boundary-readback__boundary">
        <strong>Manual-control boundary</strong>
        <span>User trigger is required. Manual acceptance is required before any future resume edit.</span>
        <span>Generated tailoring suggestions remain preview/manual-review only unless the user accepts edits in a later phase.</span>
        <span>No silent resume rewrite. No automatic resume overwrite. No resume mutation. No application submission.</span>
        <span>No auto-apply. No auto-submit. No autonomous application execution. No automatic job application submission.</span>
        <span>Manual user control required. No provider calls. No network calls. No database writes. No persistence.</span>
        <span>No mutation. No resume mutation. No application mutation. No execution. No submission.</span>
      </div>
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Action inputs", actionInputs, { helper: "Caller-supplied action-boundary inputs only." })}
        ${renderAgentTraceReadOnlyDetails("Readiness summary", result.readiness_summary || "", { helper: "Read-only advisory readiness; no tailoring generation is performed." })}
        ${renderAgentTraceReadOnlyDetails("Safety metadata", safety, { helper: "No-provider, no-network, no-database-write, no-persistence, no-mutation, no-execution, and no-submission safety metadata." })}
      </div>
          <div class="agentic-review-readonly-marker">Does not submit applications.</div>
</article>
  `;
}
...
function renderManualGenerateAiTailoringPreviewDispatchBoundaryReadbackSection(
  ...
  const result = hasAgentTraceSummaryObject(
    tracePayload?.manual_generate_ai_tailoring_preview_dispatch_boundary_result,
  )
    ? tracePayload.manual_generate_ai_tailoring_preview_dispatch_boundary_result
    : {};
  if (!Object.keys(result).length) return "";
  ...
  const readbackBadge = result.ui_api_fetch_failed === true
    ...
    : "Default-off readback";
  return `
    <article class="agent-trace-summary manual-generate-ai-tailoring-preview-dispatch-boundary-readback" aria-label="Manual Generate AI Tailoring preview dispatch-boundary readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Manual Generate AI Tailoring Preview Dispatch-Boundary Readback</h4>
          <p>UI readback only. Default-off, read-only, advisory-only, manual-review only, and dispatch-boundary contract only.</p>
          <p class="agentic-review-muted">User trigger required. Operator confirmation required. Manual acceptance required. Manual user control required. Preview/manual-review only.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(readbackBadge)}</span>
      </div>
      <div class="manual-generate-ai-tailoring-preview-dispatch-boundary-readback__safety-labels" aria-label="Manual Generate AI Tailoring preview dispatch-boundary safety labels">
        <span>Default-off</span>
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
        <span>Dispatch-boundary contract only</span>
      </div>
      <div class="agent-trace-counts manual-generate-ai-tailoring-preview-dispatch-boundary-readback__metrics">
        ${renderWorkflowSummaryMetric("Contract status", result.contract_status || "unknown")}
        ${renderWorkflowSummaryMetric("Default-off", result.default_off === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Read-only", result.read_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Advisory-only", result.advisory_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual-review only", result.manual_review_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Dispatch-boundary contract only", result.dispatch_boundary_contract_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("User trigger required", result.requires_user_trigger === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Operator confirmation required", result.operator_confirmation_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual acceptance required", result.manual_acceptance_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Dispatch ready", result.dispatch_ready === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Dispatch allowed", result.dispatch_allowed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Blocked reasons", blockedReasons.length ? blockedReasons.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Missing inputs", missingInputs.length ? missingInputs.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Request packet accepted", result.request_packet_accepted === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Deterministic dispatch key", result.deterministic_dispatch_key || "none")}
        ${renderWorkflowSummaryMetric("Does not dispatch", result.dispatch_allowed === true ? "manual readiness only" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call network", result.network_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not generate AI tailoring", result.ai_tailoring_generation_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call tailoring runtime", result.tailoring_runtime_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call providers", result.provider_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not create real tailoring output", result.real_tailoring_output_created === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not create resume rewrites", result.resume_rewrite_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not overwrite resumes", result.resume_overwrite_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not mutate resumes", result.resume_mutation_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not submit applications", result.application_submission_performed === true || result.submission_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("No provider calls", result.no_provider_calls === true && result.provider_call_performed !== true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("No network calls", result.no_network_calls === true && result.network_call_performed !== true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("No database writes", result.database_write_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("No persistence", result.persistence_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("No mutation", result.resume_mutation_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("No resume mutation", result.resume_mutation_performed === true ? "no" : "yes")}
      ...
      <div class="manual-generate-ai-tailoring-preview-dispatch-boundary-readback__boundary">
        <strong>Manual dispatch-boundary safety boundary</strong>
        <span>Does not dispatch. Does not call network. Does not generate AI tailoring.</span>
        <span>Does not call tailoring runtime. Does not call providers. Does not create real tailoring output.</span>
        <span>Does not create resume rewrites. Does not overwrite resumes. Does not mutate resumes.</span>
        <span>Does not submit applications. Does not persist data. Does not write to database.</span>
        <span>No provider calls. No network calls. No database writes. No persistence.</span>
        <span>No mutation. No resume mutation. No application mutation. No execution. No submission.</span>
      ...
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Blocked reasons", blockedReasons, { helper: "Dispatch-boundary blockers only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Missing inputs", missingInputs, { helper: "Required dispatch-boundary inputs not available for readiness." })}
        ${renderAgentTraceReadOnlyDetails("Dispatch-boundary contract", result, { helper: "Read-only dispatch-boundary contract payload returned by the API readback." })}
      </div>
    </article>
  `;
}
...
function renderManualGenerateAiTailoringPreviewProviderRequestEnvelopeReadbackSection(
  ...
  const result = hasAgentTraceSummaryObject(
    tracePayload?.manual_generate_ai_tailoring_preview_provider_request_envelope_result,
  )
    ? tracePayload.manual_generate_ai_tailoring_preview_provider_request_envelope_result
    : {};
  if (!Object.keys(result).length) return "";
  ...
  const missingInputs = Array.isArray(result.missing_inputs)
    ? result.missing_inputs
    : [];
  const requestEnvelope = hasAgentTraceSummaryObject(result.request_envelope)
  ...
  const readbackBadge = result.ui_api_fetch_failed === true
    ...
    : "Default-off readback";
  return `
    <article class="agent-trace-summary manual-generate-ai-tailoring-preview-provider-request-envelope-readback" aria-label="Manual Generate AI Tailoring preview provider request-envelope readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Manual Generate AI Tailoring Preview Provider Request-Envelope Readback</h4>
          <p>UI readback only. Default-off, read-only, advisory-only, manual-review only, and provider request-envelope contract only.</p>
          <p class="agentic-review-muted">User trigger required. Operator confirmation required. Manual acceptance required. Dispatch boundary required. Provider configuration required. Manual user control required. Preview/manual-review only.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(readbackBadge)}</span>
      </div>
      <div class="manual-generate-ai-tailoring-preview-provider-request-envelope-readback__safety-labels" aria-label="Manual Generate AI Tailoring preview provider request-envelope safety labels">
        <span>Default-off</span>
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
        <span>Provider request-envelope contract only</span>
      </div>
      <div class="agent-trace-counts manual-generate-ai-tailoring-preview-provider-request-envelope-readback__metrics">
        ${renderWorkflowSummaryMetric("Contract status", result.contract_status || "unknown")}
        ${renderWorkflowSummaryMetric("Default-off", result.default_off === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Read-only", result.read_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Advisory-only", result.advisory_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual-review only", result.manual_review_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider request-envelope contract only", result.provider_request_envelope_contract_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("User trigger required", result.requires_user_trigger === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Operator confirmation required", result.operator_confirmation_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual acceptance required", result.manual_acceptance_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Dispatch boundary required", result.dispatch_boundary_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider configuration required", result.provider_configuration_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider request envelope ready", result.provider_request_envelope_ready === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider request allowed", result.provider_request_allowed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Blocked reasons", blockedReasons.length ? blockedReasons.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Missing inputs", missingInputs.length ? missingInputs.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Request envelope", Object.keys(requestEnvelope).length ? "available" : "none")}
        ${renderWorkflowSummaryMetric("Deterministic envelope key", result.deterministic_envelope_key || requestEnvelope.deterministic_envelope_key || "none")}
        ${renderWorkflowSummaryMetric("Does not call providers", result.provider_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call network", result.network_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not dispatch", result.provider_request_allowed === true ? "manual readiness only" : "yes")}
        ${renderWorkflowSummaryMetric("Does not generate AI tailoring", result.ai_tailoring_generation_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call tailoring runtime", result.tailoring_runtime_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not create real tailoring output", result.real_tailoring_output_created === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not create resume rewrites", result.resume_rewrite_performed === true ? "no" : "yes")}
      ...
      <div class="manual-generate-ai-tailoring-preview-provider-request-envelope-readback__boundary">
        <strong>Manual provider request-envelope safety boundary</strong>
        <span>Does not call providers. Does not call network. Does not dispatch. Does not generate AI tailoring.</span>
        <span>Does not call tailoring runtime. Does not create real tailoring output. Does not create resume rewrites.</span>
        <span>Does not overwrite resumes. Does not mutate resumes. Does not submit applications.</span>
        <span>No provider calls. No network calls. No database writes. No persistence.</span>
        <span>No mutation. No resume mutation. No application mutation. No execution. No submission.</span>
        <span>No auto-apply. No auto-submit. Manual user control required. Preview/manual-review only.</span>
      </div>
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Request envelope", requestEnvelope, { helper: "Read-only provider request-envelope contract payload returned by the API readback." })}
        ${renderAgentTraceReadOnlyDetails("Blocked reasons", blockedReasons, { helper: "Provider request-envelope blockers only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Missing inputs", missingInputs, { helper: "Required provider request-envelope inputs not available for readiness." })}
      </div>
    </article>
  `;
}
...
function renderManualGenerateAiTailoringPreviewProviderCallBoundaryReadbackSection(
  ...
  const result = hasAgentTraceSummaryObject(
    tracePayload?.manual_generate_ai_tailoring_preview_provider_call_boundary_result,
  )
    ? tracePayload.manual_generate_ai_tailoring_preview_provider_call_boundary_result
    : {};
  if (!Object.keys(result).length) return "";
  ...
  const missingInputs = Array.isArray(result.missing_inputs)
    ? result.missing_inputs
    : [];
  const provider_callPlan = hasAgentTraceSummaryObject(result.provider_call_plan)
    ? result.provider_call_plan
    : {};
  const readbackBadge = result.ui_api_fetch_failed === true
    ? "Read-only fetch failure"
    : "Default-off readback";
  return `
    <article class="agent-trace-summary manual-generate-ai-tailoring-preview-provider-call-boundary-readback" aria-label="Manual Generate AI Tailoring preview provider-call boundary readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Manual Generate AI Tailoring Preview Provider-Call Boundary Readback</h4>
          <p>UI readback only. Default-off, read-only, advisory-only, manual-review only, and provider-call boundary contract only.</p>
          <p class="agentic-review-muted">User trigger required. Operator confirmation required. Manual acceptance required. Provider request-envelope required. Provider configuration required. Provider call policy required. Manual user control required. Preview/manual-review only.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(readbackBadge)}</span>
      </div>
      <div class="manual-generate-ai-tailoring-preview-provider-call-boundary-readback__safety-labels" aria-label="Manual Generate AI Tailoring preview provider-call boundary safety labels">
        <span>Default-off</span>
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
        <span>Provider-call boundary contract only</span>
      </div>
      <div class="agent-trace-counts manual-generate-ai-tailoring-preview-provider-call-boundary-readback__metrics">
        ${renderWorkflowSummaryMetric("Contract status", result.contract_status || "unknown")}
        ${renderWorkflowSummaryMetric("Default-off", result.default_off === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Read-only", result.read_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Advisory-only", result.advisory_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual-review only", result.manual_review_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider-call boundary contract only", result.provider_call_boundary_contract_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("User trigger required", result.requires_user_trigger === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Operator confirmation required", result.operator_confirmation_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual acceptance required", result.manual_acceptance_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider request-envelope required", result.provider_request_envelope_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider configuration required", result.provider_configuration_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider call policy required", result.provider_call_policy_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider call boundary ready", result.provider_call_boundary_ready === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider call allowed", result.provider_call_allowed === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Blocked reasons", blockedReasons.length ? blockedReasons.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Missing inputs", missingInputs.length ? missingInputs.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Provider call plan", Object.keys(provider_callPlan).length ? "available" : "none")}
        ${renderWorkflowSummaryMetric("Deterministic provider call key", result.deterministic_provider_call_key || provider_callPlan.deterministic_provider_call_key || "none")}
        ${renderWorkflowSummaryMetric("Does not call providers", result.provider_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call network", result.network_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not dispatch", result.dispatch_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not generate AI tailoring", result.ai_tailoring_generation_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call tailoring runtime", result.tailoring_runtime_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not create real tailoring output", result.real_tailoring_output_created === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not create resume rewrites", result.resume_rewrite_performed === true ? "no" : "yes")}
      ...
      <div class="manual-generate-ai-tailoring-preview-provider-call-boundary-readback__boundary">
        <strong>Manual provider-call boundary safety boundary</strong>
        <span>Does not call providers. Does not call network. Does not dispatch. Does not generate AI tailoring.</span>
        <span>Does not call tailoring runtime. Does not create real tailoring output. Does not create resume rewrites.</span>
        <span>Does not overwrite resumes. Does not mutate resumes. Does not submit applications.</span>
        <span>No provider calls. No network calls. No database writes. No persistence.</span>
      ...
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Provider call plan", provider_callPlan, { helper: "Read-only provider-call boundary plan returned by the API readback." })}
        ${renderAgentTraceReadOnlyDetails("Blocked reasons", blockedReasons, { helper: "Provider-call boundary blockers only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Missing inputs", missingInputs, { helper: "Required provider-call boundary inputs not available for readiness." })}
      </div>
    </article>
  `;
}
...
function renderManualGenerateAiTailoringPreviewProviderCallDryRunPacketReadbackSection(
  ...
  const result = hasAgentTraceSummaryObject(
    tracePayload?.manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_result,
  )
    ? tracePayload.manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_result
    : {};
  if (!Object.keys(result).length) return "";

  const blocked_reasons = Array.isArray(result.blocked_reasons)
    ? result.blocked_reasons
    : [];
  const missing_inputs = Array.isArray(result.missing_inputs)
    ? result.missing_inputs
    : [];
  const dry_run_packet = hasAgentTraceSummaryObject(result.dry_run_packet)
    ? result.dry_run_packet
    : {};
  const readback_badge = result.ui_api_fetch_failed === true
    ? "Read-only fetch failure"
    : "Default-off readback";
  return `
    <article class="agent-trace-summary manual-generate-ai-tailoring-preview-provider-call-dry-run-packet-readback" aria-label="Manual Generate AI Tailoring preview provider-call dry-run packet readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Manual Generate AI Tailoring Preview Provider-Call Dry-Run Packet Readback</h4>
          <p>UI readback only. Default-off, read-only, advisory-only, manual-review only, dry-run only, and provider-call dry-run packet contract only.</p>
          <p class="agentic-review-muted">User trigger required. Operator confirmation required. Manual acceptance required. Provider-call boundary required. Provider request-envelope required. Provider configuration required. Provider call policy required. Manual user control required. Preview/manual-review only.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(readback_badge)}</span>
      </div>
      <div class="manual-generate-ai-tailoring-preview-provider-call-dry-run-packet-readback__safety-labels" aria-label="Manual Generate AI Tailoring preview provider-call dry-run packet safety labels">
        <span>Default-off</span>
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
        <span>Provider-call dry-run packet contract only</span>
        <span>Dry-run only</span>
      </div>
      <div class="agent-trace-counts manual-generate-ai-tailoring-preview-provider-call-dry-run-packet-readback__metrics">
        ${renderWorkflowSummaryMetric("Contract status", result.contract_status || "unknown")}
        ${renderWorkflowSummaryMetric("Default-off", result.default_off === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Read-only", result.read_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Advisory-only", result.advisory_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual-review only", result.manual_review_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider-call dry-run packet contract only", result.provider_call_dry_run_packet_contract_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Dry-run only", result.dry_run_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("User trigger required", result.requires_user_trigger === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Operator confirmation required", result.operator_confirmation_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual acceptance required", result.manual_acceptance_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider-call boundary required", result.provider_call_boundary_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider request-envelope required", result.provider_request_envelope_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider configuration required", result.provider_configuration_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider call policy required", result.provider_call_policy_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Dry-run packet ready", result.dry_run_packet_ready === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider call allowed for future manual preview", result.provider_call_allowed_for_future_manual_preview === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Blocked reasons", blocked_reasons.length ? blocked_reasons.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Missing inputs", missing_inputs.length ? missing_inputs.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Dry-run packet", Object.keys(dry_run_packet).length ? "available" : "none")}
        ${renderWorkflowSummaryMetric("Deterministic dry-run packet key", result.deterministic_dry_run_packet_key || dry_run_packet.deterministic_dry_run_packet_key || "none")}
        ${renderWorkflowSummaryMetric("Does not call providers", result.provider_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call network", result.network_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not dispatch", result.dispatch_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not generate AI tailoring", result.ai_tailoring_generation_performed === true ? "no" : "yes")}
      ...
      <div class="manual-generate-ai-tailoring-preview-provider-call-dry-run-packet-readback__boundary">
        <strong>Manual provider-call dry-run packet safety boundary</strong>
        <span>Does not call providers. Does not call network. Does not dispatch. Does not generate AI tailoring.</span>
        <span>Does not call tailoring runtime. Does not create real tailoring output. Does not create resume rewrites.</span>
        <span>Does not overwrite resumes. Does not mutate resumes. Does not submit applications.</span>
        <span>No provider calls. No network calls. No database writes. No persistence.</span>
      ...
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Dry-run packet", dry_run_packet, { helper: "Read-only provider-call dry-run packet returned by the API readback." })}
        ${renderAgentTraceReadOnlyDetails("Blocked reasons", blocked_reasons, { helper: "Provider-call dry-run packet blockers only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Missing inputs", missing_inputs, { helper: "Required provider-call dry-run packet inputs not available for readiness." })}
      </div>
    </article>
  `;
}

function renderManualGenerateAiTailoringPreviewProviderResponseValidationReadbackSection(
  tracePayload = {},
) {
  const result = hasAgentTraceSummaryObject(
    tracePayload?.manual_generate_ai_tailoring_preview_provider_response_validation_result,
  )
    ? tracePayload.manual_generate_ai_tailoring_preview_provider_response_validation_result
    : {};
  if (!Object.keys(result).length) return "";
  ...
  const missing_inputs = Array.isArray(result.missing_inputs)
    ? result.missing_inputs
    : [];
  const validation_findings = Array.isArray(result.validation_findings)
    ? result.validation_findings
    : [];
  const provider_response_contract = hasAgentTraceSummaryObject(result.provider_response_contract)
    ? result.provider_response_contract
    : {};
  const readback_badge = result.ui_api_fetch_failed === true
    ? "Read-only fetch failure"
    : "Default-off readback";
  return `
    <article class="agent-trace-summary manual-generate-ai-tailoring-preview-provider-response-validation-readback" aria-label="Manual Generate AI Tailoring preview provider response validation readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Manual Generate AI Tailoring Preview Provider Response Validation Readback</h4>
          <p>UI readback only. Default-off, read-only, advisory-only, manual-review only, response validation only, and provider response validation contract only.</p>
          <p class="agentic-review-muted">User trigger required. Operator confirmation required. Manual acceptance required. Provider-call dry-run packet required. Provider request-envelope required. Provider response candidate required. Response validation policy required. Provider configuration required. Manual user control required. Preview/manual-review only.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(readback_badge)}</span>
      </div>
      <div class="manual-generate-ai-tailoring-preview-provider-response-validation-readback__safety-labels" aria-label="Manual Generate AI Tailoring preview provider response validation safety labels">
        <span>Default-off</span>
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
        <span>Provider response validation contract only</span>
        <span>Response validation only</span>
      </div>
      <div class="agent-trace-counts manual-generate-ai-tailoring-preview-provider-response-validation-readback__metrics">
        ${renderWorkflowSummaryMetric("Contract status", result.contract_status || "unknown")}
        ${renderWorkflowSummaryMetric("Default-off", result.default_off === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Read-only", result.read_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Advisory-only", result.advisory_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual-review only", result.manual_review_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider response validation contract only", result.provider_response_validation_contract_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Response validation only", result.response_validation_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("User trigger required", result.requires_user_trigger === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Operator confirmation required", result.operator_confirmation_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual acceptance required", result.manual_acceptance_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider-call dry-run packet required", result.provider_call_dry_run_packet_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider request-envelope required", result.provider_request_envelope_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider response candidate required", result.provider_response_candidate_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Response validation policy required", result.response_validation_policy_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider configuration required", result.provider_configuration_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Response validation ready", result.response_validation_ready === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider response accepted for future manual preview", result.provider_response_accepted_for_future_manual_preview === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Blocked reasons", blocked_reasons.length ? blocked_reasons.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Missing inputs", missing_inputs.length ? missing_inputs.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Validation findings", validation_findings.length ? validation_findings.length : "none")}
        ${renderWorkflowSummaryMetric("Provider response contract", Object.keys(provider_response_contract).length ? "available" : "none")}
        ${renderWorkflowSummaryMetric("Deterministic response validation key", result.deterministic_response_validation_key || provider_response_contract.deterministic_response_validation_key || "none")}
        ${renderWorkflowSummaryMetric("Does not call providers", result.provider_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call network", result.network_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not dispatch", result.dispatch_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not generate AI tailoring", result.ai_tailoring_generation_performed === true ? "no" : "yes")}
      ...
      <div class="manual-generate-ai-tailoring-preview-provider-response-validation-readback__boundary">
        <strong>Manual provider response validation safety boundary</strong>
        <span>Does not call providers. Does not call network. Does not dispatch. Does not generate AI tailoring.</span>
        <span>Does not call tailoring runtime. Does not create real tailoring output. Does not create resume rewrites.</span>
        <span>Does not overwrite resumes. Does not mutate resumes. Does not submit applications.</span>
        <span>No provider calls. No network calls. No database writes. No persistence.</span>
      ...
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Provider response contract", provider_response_contract, { helper: "Read-only provider response validation contract returned by the API readback." })}
        ${renderAgentTraceReadOnlyDetails("Validation findings", validation_findings, { helper: "Provider response validation findings only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Blocked reasons", blocked_reasons, { helper: "Provider response validation blockers only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Missing inputs", missing_inputs, { helper: "Required provider response validation inputs not available for readiness." })}
      </div>
    </article>
  `;
}
...
function renderManualGenerateAiTailoringPreviewProviderResponseNormalizationReadbackSection(
  ...
  const result = hasAgentTraceSummaryObject(
    tracePayload?.manual_generate_ai_tailoring_preview_provider_response_normalization_result,
  )
    ? tracePayload.manual_generate_ai_tailoring_preview_provider_response_normalization_result
    : {};
  if (!Object.keys(result).length) return "";
  ...
  const missing_inputs = Array.isArray(result.missing_inputs)
    ? result.missing_inputs
    : [];
  const normalization_findings = Array.isArray(result.normalization_findings)
    ? result.normalization_findings
    : [];
  const normalized_provider_response_contract = hasAgentTraceSummaryObject(
    result.normalized_provider_response_contract,
  )
    ? result.normalized_provider_response_contract
    : {};
  const readback_badge = result.ui_api_fetch_failed === true
    ...
    : "Default-off readback";
  return `
    <article class="agent-trace-summary manual-generate-ai-tailoring-preview-provider-response-normalization-readback" aria-label="Manual Generate AI Tailoring preview provider response normalization readback">
      <div class="agentic-workflow-header">
        <div>
          <h4>Manual Generate AI Tailoring Preview Provider Response Normalization Readback</h4>
          <p>UI readback only. Default-off, read-only, advisory-only, manual-review only, response normalization only, and provider response normalization contract only.</p>
          <p class="agentic-review-muted">User trigger required. Operator confirmation required. Manual acceptance required. Provider response validation required. Provider response candidate required. Response normalization policy required. Provider configuration required. Manual user control required. Preview/manual-review only.</p>
        </div>
        <span class="agentic-workflow-badge">${escapeHtml(readback_badge)}</span>
      </div>
      <div class="manual-generate-ai-tailoring-preview-provider-response-normalization-readback__safety-labels" aria-label="Manual Generate AI Tailoring preview provider response normalization safety labels">
        <span>Default-off</span>
        <span>Read-only</span>
        <span>Advisory-only</span>
        <span>Manual-review only</span>
        <span>Provider response normalization contract only</span>
        <span>Response normalization only</span>
      </div>
      <div class="agent-trace-counts manual-generate-ai-tailoring-preview-provider-response-normalization-readback__metrics">
        ${renderWorkflowSummaryMetric("Contract status", result.contract_status || "unknown")}
        ${renderWorkflowSummaryMetric("Default-off", result.default_off === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Read-only", result.read_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Advisory-only", result.advisory_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual-review only", result.manual_review_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider response normalization contract only", result.provider_response_normalization_contract_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Response normalization only", result.response_normalization_only === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("User trigger required", result.requires_user_trigger === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Operator confirmation required", result.operator_confirmation_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Manual acceptance required", result.manual_acceptance_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider response validation required", result.provider_response_validation_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider response candidate required", result.provider_response_candidate_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Response normalization policy required", result.response_normalization_policy_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Provider configuration required", result.provider_configuration_required === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Response normalization ready", result.response_normalization_ready === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Normalized provider response accepted for future manual preview", result.normalized_provider_response_accepted_for_future_manual_preview === true ? "yes" : "no")}
        ${renderWorkflowSummaryMetric("Blocked reasons", blocked_reasons.length ? blocked_reasons.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Missing inputs", missing_inputs.length ? missing_inputs.map(formatReviewLabel).join(", ") : "none")}
        ${renderWorkflowSummaryMetric("Normalization findings", normalization_findings.length ? normalization_findings.length : "none")}
        ${renderWorkflowSummaryMetric("Normalized provider response contract", Object.keys(normalized_provider_response_contract).length ? "available" : "none")}
        ${renderWorkflowSummaryMetric("Deterministic response normalization key", result.deterministic_response_normalization_key || normalized_provider_response_contract.deterministic_response_normalization_key || "none")}
        ${renderWorkflowSummaryMetric("Does not call providers", result.provider_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not call network", result.network_call_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not dispatch", result.dispatch_performed === true ? "no" : "yes")}
        ${renderWorkflowSummaryMetric("Does not generate AI tailoring", result.ai_tailoring_generation_performed === true ? "no" : "yes")}
      ...
      <div class="manual-generate-ai-tailoring-preview-provider-response-normalization-readback__boundary">
        <strong>Manual provider response normalization safety boundary</strong>
        <span>Does not call providers. Does not call network. Does not dispatch. Does not generate AI tailoring.</span>
        <span>Does not call tailoring runtime. Does not create real tailoring output. Does not create resume rewrites.</span>
        <span>Does not overwrite resumes. Does not mutate resumes. Does not submit applications.</span>
        <span>No provider calls. No network calls. No database writes. No persistence.</span>
      ...
      <div class="agent-trace-json-grid">
        ${renderAgentTraceReadOnlyDetails("Normalized provider response contract", normalized_provider_response_contract, { helper: "Read-only normalized provider response contract returned by the API readback." })}
        ${renderAgentTraceReadOnlyDetails("Normalization findings", normalization_findings, { helper: "Provider response normalization findings only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Blocked reasons", blocked_reasons, { helper: "Provider response normalization blockers only; no action is taken." })}
        ${renderAgentTraceReadOnlyDetails("Missing inputs", missing_inputs, { helper: "Required provider response normalization inputs not available for readiness." })}
      </div>
    </article>
  `;
}
...
function renderAgentTraceReadOnlyPanel(tracePayload = {}) {
  ...
  const tailoringAgentOpportunityVisibleTracePayload = (
    withTailoringAgentOpportunityFixture(
      coreAgentEvidenceMaterializationVisibleTracePayload,
    )
  );
  const generateAiTailoringActionBoundaryVisibleTracePayload = (
    withGenerateAiTailoringActionBoundaryFixture(
  ...
  const manualGenerateAiTailoringPreviewDispatchBoundaryVisibleTracePayload = (
    manualGenerateAiTailoringPreviewVisibleTracePayload
  );
  const manualGenerateAiTailoringPreviewProviderRequestEnvelopeVisibleTracePayload = (
    manualGenerateAiTailoringPreviewDispatchBoundaryVisibleTracePayload
  );
  const manualGenerateAiTailoringPreviewProviderCallBoundaryVisibleTracePayload = (
    manualGenerateAiTailoringPreviewProviderRequestEnvelopeVisibleTracePayload
  );
  const manualGenerateAiTailoringPreviewProviderCallDryRunPacketVisibleTracePayload = (
    manualGenerateAiTailoringPreviewProviderCallBoundaryVisibleTracePayload
  );
  const manualGenerateAiTailoringPreviewProviderResponseValidationVisibleTracePayload = (
    manualGenerateAiTailoringPreviewProviderCallDryRunPacketVisibleTracePayload
  );
  const manualGenerateAiTailoringPreviewProviderResponseNormalizationVisibleTracePayload = (
    manualGenerateAiTailoringPreviewProviderResponseValidationVisibleTracePayload
  );
...
function shouldFetchTailoringAgentOpportunityReadback(search = null) {
  const query = search === null
    ? (typeof window !== "undefined" ? window.location.search : "")
    : String(search || "");
  return new URLSearchParams(query).get(
    "tailoring_agent_opportunity_api_fetch",
  ) === "1";
}

function buildTailoringAgentOpportunityReadbackRequest(
  tracePayload = {},
) {
  const supplied = hasAgentTraceSummaryObject(
    tracePayload?.tailoring_agent_opportunity_request_payload,
  )
    ? tracePayload.tailoring_agent_opportunity_request_payload
    : {};
  if (Object.keys(supplied).length) return supplied;
  return {
    enabled: false,
    core_agent_evidence_packet: null,
    job_evidence: null,
    resume_evidence: null,
    missing_requirements: null,
    matched_terms: null,
    tailoring_context: null,
  };
}

function buildTailoringAgentOpportunityReadbackFetchFailure(error) {
  return {
    ui_api_fetch_failed: true,
    contract_status: "tailoring_agent_opportunity_readback_failed_closed",
    tailoring_agent_opportunity_contract_enabled: false,
    default_off: true,
    read_only: true,
    advisory_only: true,
    manual_review_only: true,
    ...
    no_submission: true,
    opportunity_inputs: {},
    tailoring_opportunity_summary: "Read-only API fetch failed closed.",
    tailoring_opportunities: [],
    future_user_triggered_action: "Generate AI Tailoring",
    next_safe_step: "inspect_read_only_api_fetch_failure",
    fail_closed_reason: String(
      error?.message || "tailoring_agent_opportunity_fetch_failed",
    ),
    safety_metadata: {
      read_only: true,
      advisory_only: true,
...
async function withTailoringAgentOpportunityReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  try {
    const result = await fetchJson(
      "/api/tailoring-agent-opportunity-contract",
      {
        ...
        body: JSON.stringify(
          buildTailoringAgentOpportunityReadbackRequest(source),
        ),
      },
    );
    return {
      ...source,
      tailoring_agent_opportunity_result: result,
    };
  } catch (error) {
    return {
      ...source,
      tailoring_agent_opportunity_result: (
        buildTailoringAgentOpportunityReadbackFetchFailure(error)
      ),
    };
  }
}
...
function shouldFetchGenerateAiTailoringActionBoundaryReadback(
  ...
  const query = search === null
    ...
    : String(search || "");
  return new URLSearchParams(query).get(
    "generate_ai_tailoring_action_boundary_api_fetch",
  ) === "1";
}

function buildGenerateAiTailoringActionBoundaryReadbackRequest(
  tracePayload = {},
) {
  const supplied = hasAgentTraceSummaryObject(
    tracePayload?.generate_ai_tailoring_action_boundary_request_payload,
  )
    ? tracePayload.generate_ai_tailoring_action_boundary_request_payload
    : {};
  if (Object.keys(supplied).length) return supplied;
  return {
    ...
    tailoring_agent_opportunity_payload: null,
    selected_opportunity_ids: null,
    generation_context: null,
    operator_context: null,
  };
}

function buildGenerateAiTailoringActionBoundaryReadbackFetchFailure(error) {
  return {
    ui_api_fetch_failed: true,
    action_boundary_status: (
      "generate_ai_tailoring_action_boundary_readback_failed_closed"
    ),
    generate_ai_tailoring_action_boundary_enabled: false,
    default_off: true,
    read_only: true,
    advisory_only: true,
    manual_review_only: true,
    ...
    user_triggered: false,
    action_allowed: false,
    action_blocked_reason: "read-only API fetch failed closed",
    future_action_name: "Generate AI Tailoring",
    ai_tailoring_generation_performed: false,
    tailoring_provider_call_performed: false,
    tailoring_runtime_call_performed: false,
    ...
    no_submission: true,
    action_inputs: {},
    readiness_summary: "Read-only API fetch failed closed.",
    next_safe_step: "inspect_read_only_api_fetch_failure",
    fail_closed_reason: String(
      error?.message
      || "generate_ai_tailoring_action_boundary_fetch_failed",
    ),
    safety_metadata: {
      read_only: true,
      advisory_only: true,
...
async function withGenerateAiTailoringActionBoundaryReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  try {
    const result = await fetchJson(
      "/api/generate-ai-tailoring-action-boundary",
      {
        ...
        body: JSON.stringify(
          buildGenerateAiTailoringActionBoundaryReadbackRequest(source),
        ),
      },
    );
    return {
      ...source,
      generate_ai_tailoring_action_boundary_result: result,
    };
  } catch (error) {
    return {
      ...source,
...
function shouldFetchManualGenerateAiTailoringPreviewDispatchBoundaryReadback(
  ...
  const query = search === null
    ...
    : String(search || "");
  return new URLSearchParams(query).get(
    "manual_generate_ai_tailoring_preview_dispatch_boundary_api_fetch",
  ) === "1";
}

function buildManualGenerateAiTailoringPreviewDispatchBoundaryReadbackFetchFailure(
  error,
) {
  return {
    ui_api_fetch_failed: true,
    ...
    blocked_reasons: ["read-only API fetch failed closed"],
    missing_inputs: [
      "manual_generate_ai_tailoring_preview_dispatch_boundary_contract",
    ],
    request_packet_accepted: false,
    ...
    auto_submit_performed: false,
    next_safe_step: (
      "inspect_read_only_manual_preview_dispatch_boundary_fetch_failure"
    ),
    fail_closed_reason: String(
      error?.message
      || "manual_generate_ai_tailoring_preview_dispatch_boundary_fetch_failed",
    ),
  };
}

async function withManualGenerateAiTailoringPreviewDispatchBoundaryReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  if (
    hasAgentTraceSummaryObject(
      source.manual_generate_ai_tailoring_preview_dispatch_boundary_result,
    )
    || !shouldFetchManualGenerateAiTailoringPreviewDispatchBoundaryReadback(
      search,
    )
  ) {
    return source;
  ...
  try {
    ...
    return {
      ...source,
      manual_generate_ai_tailoring_preview_dispatch_boundary_result: result,
    };
  } catch (error) {
    return {
      ...source,
      manual_generate_ai_tailoring_preview_dispatch_boundary_result: (
        buildManualGenerateAiTailoringPreviewDispatchBoundaryReadbackFetchFailure(
          error,
        )
      ),
    };
...
function shouldFetchManualGenerateAiTailoringPreviewProviderRequestEnvelopeReadback(
  ...
  const query = search === null
    ...
    : String(search || "");
  return new URLSearchParams(query).get(
    "manual_generate_ai_tailoring_preview_provider_request_envelope_api_fetch",
  ) === "1";
}

function buildManualGenerateAiTailoringPreviewProviderRequestEnvelopeReadbackFetchFailure(
  error,
) {
  return {
    ui_api_fetch_failed: true,
    ...
    dispatch_boundary_accepted: false,
    ...
    blocked_reasons: ["read-only API fetch failed closed"],
    missing_inputs: [
      "manual_generate_ai_tailoring_preview_provider_request_envelope_contract",
    ],
    request_envelope: {},
    ...
    auto_submit_performed: false,
    next_safe_step: (
      "inspect_read_only_manual_preview_provider_request_envelope_fetch_failure"
    ),
    fail_closed_reason: String(
      error?.message
      || "manual_generate_ai_tailoring_preview_provider_request_envelope_fetch_failed",
    ),
  };
}

async function withManualGenerateAiTailoringPreviewProviderRequestEnvelopeReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  if (
    hasAgentTraceSummaryObject(
      source.manual_generate_ai_tailoring_preview_provider_request_envelope_result,
    )
  ...
  try {
    ...
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_request_envelope_result: result,
    };
  } catch (error) {
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_request_envelope_result: (
        buildManualGenerateAiTailoringPreviewProviderRequestEnvelopeReadbackFetchFailure(
          error,
        )
      ),
    };
...
function shouldFetchManualGenerateAiTailoringPreviewProviderCallBoundaryReadback(
  ...
  const query = search === null
    ...
    : String(search || "");
  return new URLSearchParams(query).get(
    "manual_generate_ai_tailoring_preview_provider_call_boundary_api_fetch",
  ) === "1";
}

function buildManualGenerateAiTailoringPreviewProviderCallBoundaryReadbackFetchFailure(
  error,
) {
  return {
    ui_api_fetch_failed: true,
    ...
    provider_request_envelope_accepted: false,
    provider_configuration_required: true,
    provider_configuration_present: false,
    provider_call_policy_required: true,
    provider_call_policy_present: false,
    provider_call_boundary_ready: false,
    provider_call_allowed: false,
    blocked_reasons: ["read-only API fetch failed closed"],
    missing_inputs: [
      "manual_generate_ai_tailoring_preview_provider_call_boundary_contract",
    ],
    provider_call_plan: {},
    ...
    auto_submit_performed: false,
    next_safe_step: (
      "inspect_read_only_manual_preview_provider_call_boundary_fetch_failure"
    ),
    fail_closed_reason: String(
      error?.message
      || "manual_generate_ai_tailoring_preview_provider_call_boundary_fetch_failed",
    ),
  };
}

async function withManualGenerateAiTailoringPreviewProviderCallBoundaryReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  if (
    hasAgentTraceSummaryObject(
      source.manual_generate_ai_tailoring_preview_provider_call_boundary_result,
    )
  ...
  try {
    ...
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_call_boundary_result: result,
    };
  } catch (error) {
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_call_boundary_result: (
        buildManualGenerateAiTailoringPreviewProviderCallBoundaryReadbackFetchFailure(
          error,
        )
      ),
    };
...
function shouldFetchManualGenerateAiTailoringPreviewProviderCallDryRunPacketReadback(
  ...
  const query = search === null
    ...
    : String(search || "");
  return new URLSearchParams(query).get(
    "manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_api_fetch",
  ) === "1";
}

function buildManualGenerateAiTailoringPreviewProviderCallDryRunPacketReadbackFetchFailure(
  error,
) {
  return {
    ui_api_fetch_failed: true,
    ...
    contract_status: (
      "manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_failed_closed"
    ),
    default_off: true,
    read_only: true,
    advisory_only: true,
    manual_review_only: true,
    provider_call_dry_run_packet_contract_only: true,
    dry_run_only: true,
    requires_user_trigger: true,
    user_trigger_present: false,
    operator_confirmation_required: true,
    operator_confirmation_present: false,
    manual_acceptance_required: true,
    provider_call_boundary_required: true,
    provider_call_boundary_accepted: false,
    provider_request_envelope_required: true,
    provider_request_envelope_accepted: false,
    provider_configuration_required: true,
    provider_configuration_present: false,
    ...
    blocked_reasons: ["read-only API fetch failed closed"],
    missing_inputs: [
      "manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_contract",
    ],
    dry_run_packet: {},
    ...
    auto_submit_performed: false,
    next_safe_step: (
      "inspect_read_only_manual_preview_provider_call_dry_run_packet_fetch_failure"
    ),
    fail_closed_reason: String(
      error?.message
      || "manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_fetch_failed",
    ),
  };
}

async function withManualGenerateAiTailoringPreviewProviderCallDryRunPacketReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  if (
    hasAgentTraceSummaryObject(
      source.manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_result,
    )
  ...
  try {
    ...
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_result: result,
    };
  } catch (error) {
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_result: (
        buildManualGenerateAiTailoringPreviewProviderCallDryRunPacketReadbackFetchFailure(
          error,
        )
      ),
    };
...
function shouldFetchManualGenerateAiTailoringPreviewProviderResponseValidationReadback(
  search = null,
) {
  const query = search === null
    ? (typeof window !== "undefined" ? window.location.search : "")
    : String(search || "");
  return new URLSearchParams(query).get(
    "manual_generate_ai_tailoring_preview_provider_response_validation_api_fetch",
  ) === "1";
}

function buildManualGenerateAiTailoringPreviewProviderResponseValidationReadbackFetchFailure(
  error,
) {
  return {
    ui_api_fetch_failed: true,
    ...
    contract_status: (
      "manual_generate_ai_tailoring_preview_provider_response_validation_failed_closed"
    ),
    default_off: true,
    read_only: true,
    advisory_only: true,
    ...
    provider_call_dry_run_packet_accepted: false,
    provider_request_envelope_required: true,
    provider_request_envelope_accepted: false,
    provider_response_candidate_required: true,
    provider_response_candidate_present: false,
    response_validation_policy_required: true,
    response_validation_policy_present: false,
    provider_configuration_required: true,
    provider_configuration_present: false,
    ...
    blocked_reasons: ["read-only API fetch failed closed"],
    missing_inputs: [
      "manual_generate_ai_tailoring_preview_provider_response_validation_contract",
    ],
    validation_findings: [],
    ...
    auto_submit_performed: false,
    next_safe_step: (
      "inspect_read_only_manual_preview_provider_response_validation_fetch_failure"
    ),
    fail_closed_reason: String(
      error?.message
      || "manual_generate_ai_tailoring_preview_provider_response_validation_fetch_failed",
    ),
  };
}

async function withManualGenerateAiTailoringPreviewProviderResponseValidationReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  if (
    hasAgentTraceSummaryObject(
      source.manual_generate_ai_tailoring_preview_provider_response_validation_result,
    )
    || !shouldFetchManualGenerateAiTailoringPreviewProviderResponseValidationReadback(
      search,
    )
  ) {
    return source;
  ...
  try {
    ...
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_response_validation_result: result,
    };
  } catch (error) {
    return {
      ...source,
      manual_generate_ai_tailoring_preview_provider_response_validation_result: (
        buildManualGenerateAiTailoringPreviewProviderResponseValidationReadbackFetchFailure(
          error,
        )
      ),
    };
...
function buildManualGenerateAiTailoringPreviewProviderResponseNormalizationReadbackFetchFailure(
  error,
) {
  return {
    ui_api_fetch_failed: true,
    ...
    contract_status: (
      "manual_generate_ai_tailoring_preview_provider_response_normalization_failed_closed"
    ),
    default_off: true,
    read_only: true,
    advisory_only: true,
    ...
    provider_response_validation_accepted: false,
    provider_response_candidate_required: true,
    provider_response_candidate_present: false,
    response_normalization_policy_required: true,
    response_normalization_policy_present: false,
    provider_configuration_required: true,
    provider_configuration_present: false,
    ...
    blocked_reasons: ["read-only API fetch failed closed"],
    missing_inputs: [
      "manual_generate_ai_tailoring_preview_provider_response_normalization_contract",
    ],
    ...
    auto_submit_performed: false,
    next_safe_step: (
      "inspect_read_only_manual_preview_provider_response_normalization_fetch_failure"
    ),
    fail_closed_reason: String(
      error?.message
      || "manual_generate_ai_tailoring_preview_provider_response_normalization_fetch_failed",
    ),
  };
}

async function withManualGenerateAiTailoringPreviewProviderResponseNormalizationReadbackApiFetch(
  tracePayload = {},
  search = null,
) {
  const source = hasAgentTraceSummaryObject(tracePayload)
  ...
  if (
    hasAgentTraceSummaryObject(
      source.manual_generate_ai_tailoring_preview_provider_response_normalization_result,
    )
...
async function initAgenticReviewPage() {
  ...
  try {
    ...
    const tailoringAgentOpportunityTracePayload = await (
      withTailoringAgentOpportunityReadbackApiFetch(
        coreAgentEvidenceMaterializationTracePayload,
      )
    );
    const generateAiTailoringActionBoundaryTracePayload = await (
      withGenerateAiTailoringActionBoundaryReadbackApiFetch(
    ...
    const manualGenerateAiTailoringPreviewDispatchBoundaryTracePayload = await (
    ...
    const manualGenerateAiTailoringPreviewProviderRequestEnvelopeTracePayload = await (
      withManualGenerateAiTailoringPreviewProviderRequestEnvelopeReadbackApiFetch(
        manualGenerateAiTailoringPreviewDispatchBoundaryTracePayload,
      )
    );
    const manualGenerateAiTailoringPreviewProviderCallBoundaryTracePayload = await (
      withManualGenerateAiTailoringPreviewProviderCallBoundaryReadbackApiFetch(
        manualGenerateAiTailoringPreviewProviderRequestEnvelopeTracePayload,
      )
    );
    const manualGenerateAiTailoringPreviewProviderCallDryRunPacketTracePayload = await (
      withManualGenerateAiTailoringPreviewProviderCallDryRunPacketReadbackApiFetch(
        manualGenerateAiTailoringPreviewProviderCallBoundaryTracePayload,
      )
    );
    const manualGenerateAiTailoringPreviewProviderResponseValidationTracePayload = await (
      withManualGenerateAiTailoringPreviewProviderResponseValidationReadbackApiFetch(
        manualGenerateAiTailoringPreviewProviderCallDryRunPacketTracePayload,
      )
    );
    const manualGenerateAiTailoringPreviewProviderResponseNormalizationTracePayload = await (
      withManualGenerateAiTailoringPreviewProviderResponseNormalizationReadbackApiFetch(
        manualGenerateAiTailoringPreviewProviderResponseValidationTracePayload,
      )
    );
    if (!payload.agent_feedback) payload.agent_feedback = feedbackPayload || {};
    renderAgenticReviewData(
      payload,
      manualGenerateAiTailoringPreviewProviderResponseNormalizationTracePayload,
    );
  } catch (err) {
    const panel = qs("agenticReviewStatusCard");
    if (panel) {
