"""Default-off provider-call dry-run packet contract for manual tailoring preview."""
...
STATUS_READY = (
    "manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_ready"
)
...
def _operator_confirmation_present(metadata: dict[str, Any]) -> bool:
    ...
    return (
        ...
        or metadata.get("provider_call_dry_run_packet_confirmed") is True
    )


def _provider_call_boundary_accepted(payload: dict[str, Any]) -> bool:
    if not payload:
        return False

    nested_plan = payload.get("provider_call_plan")
    nested_plan = nested_plan if isinstance(nested_plan, dict) else {}
    boundary_allowed = (
        payload.get("provider_call_allowed") is True
        or payload.get("provider_call_boundary_ready") is True
        or nested_plan.get("provider_call_allowed") is True
        or nested_plan.get("provider_call_boundary_ready") is True
    )
    no_prior_activity = (
    ...
    return boundary_allowed and no_prior_activity
...
def _deterministic_key(payload: dict[str, Any]) -> str:
    ...
    return "manual-generate-ai-tailoring-preview-provider-dry-run-" + sha256(
        encoded.encode("utf-8")
    ).hexdigest()[:24]


def build_manual_generate_ai_tailoring_preview_provider_call_dry_run_packet_contract(
    *,
    ...
    """Return dry-run packet readiness metadata without live side effects."""
    ...
    provider_call_boundary_accepted = _provider_call_boundary_accepted(
        safe_provider_call_boundary
    )
    ...
    if not safe_provider_call_boundary:
        missing_inputs.append("phase28_provider_call_boundary_payload")
    ...
    if not safe_provider_call_boundary:
        blocked_reasons.append("phase28 provider-call boundary payload required")
    elif not provider_call_boundary_accepted:
    ...
    dry_run_packet_ready = (
        ...
        and provider_call_boundary_accepted
    ...
    provider_call_allowed_for_future_manual_preview = dry_run_packet_ready
    contract_status = STATUS_READY if dry_run_packet_ready else STATUS_BLOCKED
    ...
    if not user_trigger_present:
    ...
    elif not safe_provider_call_boundary or not provider_call_boundary_accepted:
        next_safe_step = "provide_accepted_provider_call_boundary"
        ...
        next_safe_step = (
            "manual_review_provider_call_dry_run_packet_without_live_call"
        )
    ...
    dry_run_inputs = {
        ...
        "provider_call_boundary_accepted": provider_call_boundary_accepted,
        ...
        "dry_run_packet_ready": dry_run_packet_ready,
    }
    deterministic_dry_run_packet_key = _deterministic_key(dry_run_inputs)

    dry_run_packet = {
        "dry_run_packet_status": contract_status,
        "deterministic_dry_run_packet_key": deterministic_dry_run_packet_key,
        "read_only": True,
        "advisory_only": True,
        ...
        "source_inputs": deepcopy(dry_run_inputs),
    }
