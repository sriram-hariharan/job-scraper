...
This is a release checkpoint for the Phase 27 manual Generate AI Tailoring preview provider request-envelope slice.

Phase 27D is docs/tests only. It makes no runtime behavior changes, no backend behavior changes, no API changes, no UI changes, no services changes, no agent helper changes, no pipeline changes, no matching changes, and no tailoring runtime changes.

## Phase 27 slice summary
...
This release remains default-off, read-only, advisory-only, manual-review only, and provider request-envelope contract only. User trigger required. Operator confirmation required. Manual acceptance required. Dispatch boundary required. Provider configuration required.
