"""Dedicated two-row durable evidence runtime for Groq canary run 004."""
...
from src.evaluation.controlled_groq_canary_run_004_identity import (
    ...
    RUN_004_AUTHORIZATION_TEMPLATE_VERSION,
    RUN_004_IDENTIFIER,
    RUN_004_IDENTITY_VERSION,
    build_run_004_authorization_template,
    build_run_004_identity_contract,
    run_004_authorization_template_sha256,
    run_004_identity_sha256,
    validate_run_004_authorization_template,
    validate_run_004_identity_contract,
)
from src.evaluation.controlled_groq_canary_run_004_plan import (
    RUN_004_PLAN_VERSION,
    build_run_004_plan_contract,
    run_004_plan_sha256,
    validate_run_004_plan_contract,
)
...
RUN_004_EVIDENCE_RUNTIME_VERSION = (
    "controlled-groq-canary-run-004-evidence-runtime-v1"
)
RUN_004_CHECKPOINT_SCHEMA_VERSION = (
    "controlled-groq-canary-run-004-checkpoint-v1"
)
RUN_004_RESULT_SCHEMA_VERSION = "controlled-groq-canary-run-004-result-v1"

_PINNED_PLAN_SHA256 = (
    "8ce5d173205dfa30e88a0542b73a0fcd85d82fa525992a1265ee2ac471ab44ae"
)
_PINNED_IDENTITY_SHA256 = (
    "6c4e518cdbcd252ecd2c321ac46bc54ebe2ea2a2400659aa5be6574c8ee0c018"
)
_PINNED_AUTHORIZATION_TEMPLATE_SHA256 = (
    "16683e02bec79fb5f0291fb1facc7ddf2578e6e8abbbce6404d496fb3269a61c"
)
...
_CHECKPOINT_FIELDS = {
    ...
    "run_004_identity_version",
    "run_004_identity_sha256",
    "run_004_transport_version",
    "run_004_transport_sha256",
...
def _validated_owners() -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    identity = build_run_004_identity_contract()
    validate_run_004_plan_contract(plan)
    validate_run_004_identity_contract(identity)
    _require(
        run_004_plan_sha256(plan) == _PINNED_PLAN_SHA256,
        "run-004 plan digest changed",
    )
    _require(
        run_004_identity_sha256(identity) == _PINNED_IDENTITY_SHA256,
        "run-004 identity digest changed",
    )
    _require(
        run_004_authorization_template_sha256()
        == _PINNED_AUTHORIZATION_TEMPLATE_SHA256,
    ...
    _require(
        ...
        == _PINNED_SCHEDULE_KEYS,
        "run-004 schedule ownership changed",
    )
    return deepcopy(plan), deepcopy(identity)


def validate_run_004_active_authorization(
    ...
    *,
    ...
    _require(
        ...
        "active run-004 authorization is required",
    )
    inactive = build_run_004_authorization_template()
    validate_run_004_authorization_template(inactive)
    _require(
        set(authorization) == set(inactive),
        "run-004 authorization fields must match the exact template",
    )
    for key in set(inactive) - _ACTIVE_AUTHORIZATION_FIELDS:
        _require(
            authorization[key] == inactive[key],
            f"run-004 authorization changed fixed field {key}",
        )
...
def _bindings(
    *,
    ...
    return {
        ...
        "checkpoint_schema_version": RUN_004_CHECKPOINT_SCHEMA_VERSION,
        "run_identifier": RUN_004_IDENTIFIER,
        "run_004_plan_version": RUN_004_PLAN_VERSION,
        "run_004_plan_sha256": _PINNED_PLAN_SHA256,
        "run_004_identity_version": RUN_004_IDENTITY_VERSION,
        "run_004_identity_sha256": _PINNED_IDENTITY_SHA256,
        "run_004_transport_version": TRANSPORT_VERSION,
        "run_004_transport_sha256": _PINNED_TRANSPORT_SHA256,
...
def build_empty_run_004_checkpoint(
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
    execution_at_utc: str,
) -> Dict[str, Any]:
    validate_run_004_active_authorization(
        authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
    )
...
def _state_sets(checkpoint: Mapping[str, Any]) -> tuple[set[str], ...]:
    ...
    for field in _STATE_FIELDS:
        ...
        _require(
            ...
            f"{field} contains invalid run-004 keys",
        )
        observed.append(set(values))
    for index, left in enumerate(observed):
        for right in observed[index + 1 :]:
            _require(left.isdisjoint(right), "run-004 state lists overlap")
...
def _validate_summary(summary: Dict[str, Any]) -> bool:
    _require(
        ...
        "run-004 grading summary fields are invalid",
    )
    ownership = {
        row["schedule_key"]: row
        for row in build_run_004_plan_contract()["schedule"]
    }
    ...
    _require(
        ...
        "run-004 grading summary ownership changed",
    )
    _require(
        ...
        "run-004 grading booleans are invalid",
    )
    ...
    _require(
        ...
        "run-004 hard failures are not bounded positive counters",
    )
    _require(
        summary["provider_outcome_category"] == "success",
        "run-004 persisted outcome category is invalid",
    )
...
def validate_run_004_checkpoint(
    ...
    *,
    ...
    _require(
        ...
        "run-004 checkpoint fields must match the exact schema",
    )
    _require(
        not _contains_prohibited_evidence(checkpoint),
        "run-004 checkpoint contains prohibited evidence",
    )
    bindings = _bindings(authorization=authorization, pricing=pricing)
    for key, expected in bindings.items():
        _require(
            checkpoint[key] == expected,
            f"run-004 checkpoint binding changed: {key}",
        )
    states = _state_sets(checkpoint)
    invoked = set().union(*states)
    _require(len(invoked) <= 2, "run-004 cannot invoke more than two keys")
    summaries = checkpoint["grading_summaries"]
    _require(
        isinstance(summaries, list) and len(summaries) <= 2,
        "run-004 grading summaries are invalid",
    )
    ...
    _require(
        ...
        "completed run-004 keys must be an ordered prefix",
    )
    failure_count = len(blocked | ambiguous | hard)
    _require(failure_count <= 1, "run-004 has multiple terminal failures")
    if failure_count:
        ...
        _require(
            ...
            "run-004 failure key is not the exact next row",
        )
    ...
    _require(
        ...
        "run-004 grading summaries are reordered",
    )
    if not invoked:
        _require(
            summaries == []
            ...
            "empty run-004 checkpoint state is inconsistent",
        )
    elif len(completed) == 1 and not failure_count:
        _require(
            len(summaries) == 1
            ...
            and checkpoint["quality_gate_status"] == "pending"
            and checkpoint["cost_comparison_eligibility"] is False,
            "partially completed run-004 checkpoint is inconsistent",
        )
        ...
        _require(
            ...
            "fully completed run-004 checkpoint is inconsistent",
        )
    elif hard:
        _require(
            ...
            "hard-failure run-004 checkpoint is inconsistent",
        )
    elif blocked:
        _require(
            len(summaries) == len(completed)
            ...
            and checkpoint["cost_comparison_eligibility"] is False,
            "blocked run-004 checkpoint is inconsistent",
        )
    elif ambiguous:
        _require(
            len(summaries) == len(completed)
            ...
            and checkpoint["quality_gate_status"] == "stopped"
            and checkpoint["cost_comparison_eligibility"] is False,
            "ambiguous run-004 checkpoint is inconsistent",
        )
    ...
    _require(
        ...
        "run-004 aggregate usage is inconsistent",
    )
    _require(
        checkpoint["authority_invariants"] == _expected_authority(len(invoked)),
        "run-004 checkpoint authority changed",
    )
    return True


def get_next_run_004_row(
    checkpoint: Dict[str, Any],
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
    ...
    if (
        ...
        or len(checkpoint["completed_schedule_keys"]) == 2
    ):
        return None
    return deepcopy(
...
def _validate_transition_start(
    ...
    *,
    ...
    next_row = get_next_run_004_row(
        checkpoint,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
    )
    _require(next_row is not None, "run-004 checkpoint is terminal")
    _require(
        isinstance(scheduled, Mapping) and dict(scheduled) == next_row,
        "run-004 transition row is not the exact next row",
    )
...
def _terminal_transition(
    ...
    *,
    ...
    fully_completed = (
        ...
        and len(updated["completed_schedule_keys"]) == 2
    )
...
def record_run_004_blocked_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    authorization: Dict[str, Any],
...
def record_run_004_ambiguous_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    authorization: Dict[str, Any],
    ...
    _require(
        ...
        "run-004 ambiguous reason is not approved",
    )
    return _terminal_transition(
        checkpoint,
...
def record_run_004_hard_failure_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    authorization: Dict[str, Any],
...
def record_run_004_completed_call(
    checkpoint: Dict[str, Any],
    *,
    scheduled: Mapping[str, Any],
    ...
    if (
        ...
        return record_run_004_hard_failure_call(
            checkpoint,
            scheduled=row,
            authorization=authorization,
            pricing=pricing,
    ...
    _require(
        ...
        "run-004 transport result fields are invalid",
    )
    _require(
        transport_result["provider_outcome_category"] == "success",
        "run-004 completed transition requires a success outcome",
    )
    if not (
    ...
    _require(len(matches) == 1, "run-004 fixture ownership changed")
    case = matches[0]
    ...
    token_exceeded = (
        ...
        or prior_aggregate["input_token_count"] + input_tokens > 8192
        or prior_aggregate["output_token_count"] + output_tokens > 2048
    )
    if token_exceeded:
...
def serialize_run_004_checkpoint(
    checkpoint: Dict[str, Any],
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
...
def build_run_004_result_artifact(
    *,
    checkpoint: Dict[str, Any],
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
    ...
    _require(
        ...
        "result requires terminal run-004 state",
    )
    ...
    artifact = {
        "result_schema_version": RUN_004_RESULT_SCHEMA_VERSION,
        "run_004_evidence_runtime_version": (
            RUN_004_EVIDENCE_RUNTIME_VERSION
        ),
        "run_identifier": RUN_004_IDENTIFIER,
        ...
        "authorization_sha256": _canonical_sha256(authorization),
        "pricing_sha256": pricing_table_sha256(pricing),
        "artifact_paths": deepcopy(RUN_004_ARTIFACT_PATHS),
    ...
    validate_run_004_result_artifact(
        artifact,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
...
def validate_run_004_result_artifact(
    ...
    *,
    ...
    _require(
        ...
        "run-004 result fields must match the exact schema",
    )
    _require(
        not _contains_prohibited_evidence(artifact),
        "run-004 result contains prohibited evidence",
    )
    ...
    expected_fixed = {
        ...
        "authorization_sha256": _canonical_sha256(authorization),
        "pricing_sha256": pricing_table_sha256(pricing),
        "artifact_paths": RUN_004_ARTIFACT_PATHS,
    }
    ...
    _require(
        ...
        "run-004 result and checkpoint disagree",
    )
    _require(
        ...
        "run-004 result authority changed",
    )
    _require(
        ...
        "run-004 retention policy changed",
    )
    return True


def serialize_run_004_result_artifact(
    artifact: Dict[str, Any],
    *,
    authorization: Dict[str, Any],
    pricing: Dict[str, Any],
...
def run_004_result_sha256(
    ...
    *,
    ...
    return sha256(
        serialize_run_004_result_artifact(
            artifact,
            authorization=authorization,
            pricing=pricing,
            execution_at_utc=execution_at_utc,
...
def _validate_artifact_path(
    ...
    *,
    ...
    expected = root / RUN_004_ARTIFACT_PATHS[kind]
    _require(
        candidate == expected,
        f"only the exact run-004 {kind} path is allowed",
    )
...
def load_run_004_checkpoint(
    ...
    *,
    ...
    _require(
        ...
        "run-004 checkpoint mode must be 0600",
    )
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        raise ValueError("persisted run-004 checkpoint is malformed") from None
    validate_run_004_checkpoint(
        payload,
        authorization=authorization,
        pricing=pricing,
...
def write_initial_run_004_checkpoint(
    checkpoint_path: str | Path,
    checkpoint: Dict[str, Any],
    *,
    repository_root: str | Path,
    ...
    encoded = serialize_run_004_checkpoint(
        checkpoint,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
...
def replace_run_004_checkpoint_atomic(
    checkpoint_path: str | Path,
    checkpoint: Dict[str, Any],
    *,
    ...
    observed_prior = run_004_checkpoint_sha256(
        prior,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
    ...
    _require(
        ...
        "run-004 checkpoint prior digest mismatch",
    )
    encoded = serialize_run_004_checkpoint(
        checkpoint,
        authorization=authorization,
    ...
    try:
        ...
        os.close(descriptor)
        descriptor = -1
        current = load_run_004_checkpoint(
            path,
            repository_root=repository_root,
            authorization=authorization,
            pricing=pricing,
        ...
        current_sha = run_004_checkpoint_sha256(
            current,
            authorization=authorization,
            pricing=pricing,
            execution_at_utc=execution_at_utc,
        ...
        _require(
            ...
            "run-004 checkpoint changed before replacement",
        )
        ...
        try:
            ...
            pass
        raise
    loaded = load_run_004_checkpoint(
        path,
        repository_root=repository_root,
        authorization=authorization,
        pricing=pricing,
    ...
    _require(loaded == checkpoint, "run-004 checkpoint replacement changed")
    return path


def load_run_004_result_artifact(
    ...
    *,
    ...
    _require(
        stat.S_IMODE(path.stat().st_mode) == 0o600,
        "run-004 result mode must be 0600",
    )
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        raise ValueError("persisted run-004 result is malformed") from None
    validate_run_004_result_artifact(
        payload,
        authorization=authorization,
        pricing=pricing,
...
def write_run_004_result_exclusive(
    result_path: str | Path,
    artifact: Dict[str, Any],
    *,
    repository_root: str | Path,
    ...
    encoded = serialize_run_004_result_artifact(
        artifact,
        authorization=authorization,
        pricing=pricing,
        execution_at_utc=execution_at_utc,
