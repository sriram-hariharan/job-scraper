...
Phase 28C adds UI readback only for the Phase 28B manual Generate AI Tailoring preview provider-call boundary API readback.

The UI passively displays the read-only payload from `/api/manual-generate-ai-tailoring-preview-provider-call-boundary-contract` through `renderManualGenerateAiTailoringPreviewProviderCallBoundaryReadbackSection`. It does not add a real Generate AI Tailoring button or control, provider-call control, network-call control, or dispatch control.

## Readback boundary

This UI readback is provider-call boundary contract only, default-off, read-only, advisory-only, and manual-review only. User trigger required. Operator confirmation required. Manual acceptance required. Provider request-envelope required. Provider configuration required. Provider call policy required.
...
## Exact Phase 28C safety markers
